...
describe('@function city-colour', () => {
  ...
  it('throws a deprecation warning if the $legacy parameter is used', async () => {
    const sass = `
      ...
      .foo {
        color: city-colour('red', $legacy: 'blue');
      }
    `
    ...
    expect(mockWarnFunction.mock.calls[0]).toEqual(
      expect.arrayContaining([
        'The `$legacy` parameter of `city-colour` is deprecated and is ' +
          'non-operational. It will be removed in the next major version. To ' +
          'silence this warning, update $city-suppressed-warnings with key: ' +
          '"legacy-colour-param"'
      ])
    )
  })
...
describe('@function city-organisation-colour', () => {
  const sassBootstrap = `
    $city-colours-organisations: (
  ...
  it('returns the websafe colour for a given organisation by default', async () => {
    const sass = `
      ...
      .foo {
        color: city-organisation-colour('floo-network-authority');
      }
    `
  ...
  it('falls back to the default colour if a websafe colour is not explicitly defined', async () => {
    const sass = `
      ...
      .foo {
        color: city-organisation-colour('broom-regulatory-control');
      }
    `
  ...
  it('can be overridden to return the non-websafe colour', async () => {
    const sass = `
      ...
      .foo {
        border-color: city-organisation-colour('floo-network-authority', $websafe: false);
      }
    `
  ...
  it('throws an error if a non-existent organisation is requested', async () => {
    const sass = `
      ...
      .foo {
        color: city-organisation-colour('muggle-born-registration-commission');
      }
    `
