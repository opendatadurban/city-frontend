...
} from '@city-frontend/lib/names'

import * as cityFrontend from './all.mjs'
...
describe('initAll', () => {
  ...
  it.each(components)(
    ...
    (componentName) => {
      ...
      document.body.innerHTML = `<div data-module="city-${componentName}"></div>`

      initAll()

      expect(cityFrontend[className]).toHaveBeenCalledWith(
        document.querySelector(`[data-module="city-${componentName}"]`)
      )
    }
  )
  ...
  describe('city-frontend-supported not present', () => {
    beforeAll(() => {
    ...
    it('returns early', () => {
      document.body.innerHTML = '<div data-module="city-accordion"></div>'

      initAll()

      expect(cityFrontend.Accordion).not.toHaveBeenCalled()
    })
  ...
  it('only initialises components within a given scope', () => {
    document.body.classList.add('city-frontend-supported')
    document.body.innerHTML = `
      <div data-module="city-accordion"></div>
    ...
    expect(cityFrontend.Accordion).toHaveBeenCalledTimes(1)
    expect(cityFrontend.Accordion).toHaveBeenCalledWith(
      document.querySelector('.my-scope [data-module="city-accordion"]')
    )
  })
  ...
  it('catches errors thrown by components and logs them to the console', () => {
    ...
    jest.mocked(cityFrontend.Accordion).mockImplementation(() => {
