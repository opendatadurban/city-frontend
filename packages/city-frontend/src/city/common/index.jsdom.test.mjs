...
describe('Common JS utilities', () => {
  ...
  describe('isSupported', () => {
    ...
    it('returns true if the city-frontend-supported class is set', () => {
      document.body.classList.add('city-frontend-supported')

      expect(isSupported(document.body)).toBe(true)
    })

    it('returns false if the city-frontend-supported class is not set', () => {
  ...
  describe('getBreakpoint', () => {
    beforeAll(() => {
      ...
      stylesheet.innerHTML = outdent`
        :root {
          --city-frontend-breakpoint-mobile: 40em;
          --city-frontend-breakpoint-tablet: 80em;
        }

        body {
          --city-frontend-breakpoint-tablet: 90em;
        }
      `
    ...
    it('returns the breakpoint value if it exists', () => {
      expect(getBreakpoint('mobile')).toEqual({
        property: '--city-frontend-breakpoint-mobile',
        value: '40em'
      })
    })
    ...
    it('returns the value as set on the HTML (root) element', () => {
      expect(getBreakpoint('tablet')).toEqual({
        property: '--city-frontend-breakpoint-tablet',
        value: '80em'
      })
    })
    ...
    it('returns an undefined value if the breakpoint does not exist', () => {
      expect(getBreakpoint('giant-video-wall')).toEqual({
        property: '--city-frontend-breakpoint-giant-video-wall',
        value: undefined
      })
    })
  })
