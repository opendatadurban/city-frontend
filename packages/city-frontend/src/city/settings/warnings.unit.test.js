...
describe('Warnings mixin', () => {
    ...
    expect(mockWarnFunction.mock.calls[0]).toEqual(
      expect.arrayContaining([
        'This is a warning. To silence this warning, update ' +
          '$city-suppressed-warnings with key: "test"'
      ])
    )
  })
  ...
  it('Does not fire a @warn if the key is already in $city-suppressed-warnings', async () => {
    const sass = `
      ${sassBootstrap}

      $city-suppressed-warnings: append($city-suppressed-warnings, 'test');
      @include _warning('test', 'This is a warning.');
    `
