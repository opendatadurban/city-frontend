...
const { goToExample, render } = require('@city-frontend/helpers/puppeteer')
const { getExamples } = require('@city-frontend/lib/components')

const buttonClass = '.city-js-exit-this-page-button'
const skiplinkClass = '.city-js-exit-this-page-skiplink'
const overlayClass = '.city-exit-this-page-overlay'
        ...
        await expect(
          render(page, 'exit-this-page', examples.default, {
            beforeInitialisation($module) {
              ...
              $module.outerHTML = `<svg data-module="city-exit-this-page"></svg>`
            }
          })
        ).rejects.toMatchObject({
          cause: {
      ...
      it('throws when the button is missing', async () => {
        await expect(
          render(page, 'exit-this-page', examples.default, {
            ...
            context: {
              selector: '.city-exit-this-page__button'
            }
          })
        ).rejects.toMatchObject({
          cause: {
            name: 'ElementError',
            message:
              'Exit this page: Button (`.city-exit-this-page__button`) not found'
          }
        })
      })
    })
