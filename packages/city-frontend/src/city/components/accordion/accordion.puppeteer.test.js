const {
  ...
  render,
  getAccessibleName
} = require('@city-frontend/helpers/puppeteer')
  ...
  describe('/components/accordion/preview', () => {
      ...
      it('falls back to making all accordion sections visible', async () => {
        ...
        for (let i = 0; i < numberOfExampleSections; i++) {
          const isContentVisible = await page.waitForSelector(
            `.city-accordion .city-accordion__section:nth-of-type(${
              i + 1
            }) .city-accordion__section-content`,
        ...
        const numberOfIcons = await page.evaluate(
          () =>
            document.body.querySelectorAll(
              '.city-accordion .city-accordion__section .city-accordion-nav__chevron'
            ).length
        )
    ...
    describe('when JavaScript is available', () => {
          ...
          const sectionHeaderButtonExpanded = await page.evaluate(function (i) {
            return document.body
              .querySelector(
                `.city-accordion .city-accordion__section:nth-of-type(${
                  2 + i
                }) .city-accordion__section-button`
              )
      ...
      it('should change the Show all sections button to Hide all sections when both sections are opened', async () => {
        ...
        await page.click(
          '.city-accordion .city-accordion__section:nth-of-type(2) .city-accordion__section-header'
        )
        await page.click(
          '.city-accordion .city-accordion__section:nth-of-type(3) .city-accordion__section-header'
        )

        const openOrCloseAllButtonText = await page.evaluate(
          () =>
            document.body.querySelector('.city-accordion__show-all').textContent
        )
        await page.click('.city-accordion__show-all')
      ...
      it('should open both sections when the Show all sections button is clicked', async () => {
        ...
        const firstSectionHeaderButtonExpanded = await page.evaluate(() =>
          document.body
            .querySelectorAll('.city-accordion__section')
            .item(0)
            .querySelector('.city-accordion__section-button')
            .getAttribute('aria-expanded')
        )
      ...
      it('should maintain the expanded state after a page refresh', async () => {
        const sectionHeaderButton =
          '.city-accordion .city-accordion__section:nth-of-type(2) .city-accordion__section-button'
        ...
        const buttonTag = await page.evaluate(
          () =>
            document.body.querySelector(
              '.city-accordion .city-accordion__section-button'
            ).tagName
        )
      ...
      it('should contain a heading text container', async () => {
        ...
        const headingTextContainer = await page.evaluate(() =>
          document.body.querySelector(
            '.city-accordion .city-accordion__section-button > .city-accordion__section-heading-text'
          )
        )
      ...
      describe('focus containers', () => {
        it('should contain a heading text focus container', async () => {
          ...
          const headingTextFocusContainer = await page.evaluate(() =>
            document.body.querySelector(
              '.city-accordion .city-accordion__section-button .city-accordion__section-heading-text > .city-accordion__section-heading-text-focus'
            )
          )
        ...
        it('should contain a summary focus container', async () => {
          ...
          const summaryFocusContainer = await page.evaluate(() =>
            document.body.querySelector(
              '.city-accordion .city-accordion__section-button > .city-accordion__section-summary > .city-accordion__section-summary-focus'
            )
          )
        ...
        it('should contain a show/hide focus container', async () => {
          ...
          const headingTextFocusContainer = await page.evaluate(() =>
            document.body.querySelector(
              '.city-accordion .city-accordion__section-button .city-accordion__section-toggle > .city-accordion__section-toggle-focus'
            )
          )
      ...
      describe('hidden comma', () => {
        it('should contain hidden comma " ," after the heading text for when CSS does not load', async () => {
          ...
          const commaAfterHeadingTextClassName = await page.evaluate(
            () =>
              document.body.querySelector(
                '.city-accordion__section-heading-text'
          ...
          expect(commaAfterHeadingTextClassName).toBe(
            'city-visually-hidden city-accordion__section-heading-divider'
          )
        ...
        it('should contain hidden comma " ," after the summary line for when CSS does not load', async () => {
          ...
          const commaAfterHeadingTextClassName = await page.evaluate(
            () =>
              document.body.querySelector('.city-accordion__section-summary')
                .nextElementSibling.className
          )
      ...
      describe('summary line', () => {
        describe('location of summary', () => {
          it('should move the additional description to the button text in the correct order', async () => {
            ...
            const summaryClass = 'city-accordion__section-summary city-body'
            const firstSummaryElement = await page.evaluate(
              () =>
                document.body.querySelectorAll(
                  '.city-accordion__section-button > span'
                )[2].className
            )
        ...
        describe('div to span', () => {
          it('should have converted the div to a span tag', async () => {
            ...
            const firstSummaryElement = await page.evaluate(
              () =>
                document.body.querySelector(
                  '.city-accordion .city-accordion__section .city-accordion__section-summary'
                ).outerHTML
            )
      ...
      it('should change the Show text to Hide when sections are opened', async () => {
        ...
        const ShowOrHideButtonText = await page.evaluate(
          () =>
            document.body.querySelector('.city-accordion__section-toggle-text')
              .textContent
        )
      ...
      it('should have a data-nosnippet attribute on the "Show / hide" container to hide it from search result snippets', async () => {
        ...
        const dataNoSnippetAttribute = await page.evaluate(() =>
          document.body
            .querySelector('.city-accordion__section-toggle')
      ...
      describe('accessible name', () => {
        it('Should set the accessible name of the show/hide section buttons', async () => {
          ...
          const $element = await page.$('.city-accordion__section-button')
      ...
      describe('localisation', () => {
        ...
        it('should localise "Hide all sections" based on JavaScript configuration', async () => {
          const page = await goToExample(browser, 'translated')
          await page.click('.city-accordion .city-accordion__show-all')
        ...
        it('should localise "Show section" based on data attribute', async () => {
          ...
          const firstSectionToggleText = await page.evaluate(
            () =>
              document.body.querySelector(
                '.city-accordion__section-toggle-text'
              ).innerHTML
          )
          ...
          it('injects the localised strings as text not HTML', async () => {
            ...
            const { toggleAllContent, firstSectionToggleContent } =
              await page.$eval('[data-module]', (el) => ({
                toggleAllContent: el
                  .querySelector('.city-accordion__show-all-text')
                  .innerHTML.trim(),
                firstSectionToggleContent: el
                  .querySelector('.city-accordion__section-toggle-text')
                  .innerHTML.trim()
              }))
          ...
          await expect(
            render(page, 'accordion', examples.default, {
              beforeInitialisation($module) {
                ...
                $module.outerHTML = `<svg data-module="city-accordion"></svg>`
              }
            })
          ).rejects.toMatchObject({
            cause: {
        ...
        it('throws when the accordion sections are missing', async () => {
          await expect(
            render(page, 'accordion', examples.default, {
              ...
              context: {
                selector: '.city-accordion__section'
              }
            })
          ).rejects.toMatchObject({
            cause: {
              name: 'ElementError',
              message:
                'Accordion: Sections (`<div class="city-accordion__section">`) not found'
            }
          })
        })
        ...
        it('throws when section header is missing', async () => {
          await expect(
            render(page, 'accordion', examples.default, {
              ...
              context: {
                selector: '.city-accordion__section-header'
              }
            })
          ).rejects.toMatchObject({
            cause: {
              name: 'ElementError',
              message:
                'Accordion: Section headers (`<div class="city-accordion__section-header">`) not found'
            }
          })
        })
        ...
        it('throws when any section heading is missing', async () => {
          await expect(
            render(page, 'accordion', examples.default, {
              ...
              context: {
                selector: '.city-accordion__section-heading'
              }
            })
          ).rejects.toMatchObject({
            cause: {
              name: 'ElementError',
              message:
                'Accordion: Section heading (`.city-accordion__section-heading`) not found'
            }
          })
        })
        ...
        it('throws when any section button placeholder span is missing', async () => {
          await expect(
            render(page, 'accordion', examples.default, {
              ...
              context: {
                selector: '.city-accordion__section-button'
              }
            })
          ).rejects.toMatchObject({
            cause: {
              name: 'ElementError',
              message:
                'Accordion: Section button placeholder (`<span class="city-accordion__section-button">`) not found'
            }
          })
        })
        ...
        it('throws when any section content is missing', async () => {
          await expect(
            render(page, 'accordion', examples.default, {
              ...
              context: {
                selector: '.city-accordion__section-content'
              }
            })
          ).rejects.toMatchObject({
            cause: {
              name: 'ElementError',
              message:
                'Accordion: Section content (`<div class="city-accordion__section-content">`) not found'
            }
          })
        })
      })
