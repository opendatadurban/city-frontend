...
describe('Radios', () => {
  ...
  describe('input position', () => {
    ...
    it('displays the input above the label', async () => {
      ...
      const $firstInput = await page.$('.city-radios__input')
    ...
    describe('when JavaScript is unavailable or fails', () => {
      ...
      describe('with conditional items', () => {
        ...
        beforeAll(async () => {
          ...
          $component = await page.$('.city-radios')
          $inputs = await $component.$$('.city-radios__input')
          $conditionals = await $component.$$('.city-radios__conditional')
        })
        ...
        it('has no ARIA attributes applied', async () => {
          const $inputsWithAriaExpanded = await $component.$$(
            '.city-radios__input[aria-expanded]'
          )
          const $inputsWithAriaControls = await $component.$$(
            '.city-radios__input[aria-controls]'
          )
  ...
  describe('with multiple groups', () => {
    describe('when JavaScript is available', () => {
      ...
      beforeEach(async () => {
        ...
        $inputsWarm = await page.$$('.city-radios__input[id^="warm"]')
        $inputsCool = await page.$$('.city-radios__input[id^="cool"]')
        $inputsNotInForm = await page.$$(
          '.city-radios__input[id^="question-not-in-form"]'
        )
      })
    ...
    describe('when JavaScript is available', () => {
      ...
      beforeEach(async () => {
        ...
        $inputsPrimary = await page.$$(
          '.city-radios__input[id^="fave-primary"]'
        )
        $inputsOther = await page.$$('.city-radios__input[id^="fave-other"]')
      })
      ...
      await expect(
        render(page, 'radios', examples.default, {
          beforeInitialisation($module) {
            ...
            $module.outerHTML = `<svg data-module="city-radios"></svg>`
          }
        })
      ).rejects.toMatchObject({
        cause: {
    ...
    it('throws when a conditional target element is not found', async () => {
      await expect(
        render(page, 'radios', examples['with conditional items'], {
          beforeInitialisation($module) {
            $module.querySelector('.city-radios__conditional').remove()
          }
        })
      ).rejects.toMatchObject({
        cause: {
