...
describe('Checkboxes', () => {
  ...
  describe('input position', () => {
    ...
    it('displays the input above the label', async () => {
      ...
      const $firstInput = await page.$('.city-checkboxes__input')
    ...
    describe('when JavaScript is unavailable or fails', () => {
      ...
      describe('with conditional items', () => {
        ...
        beforeAll(async () => {
          ...
          $component = await page.$('.city-checkboxes')
          $inputs = await $component.$$('.city-checkboxes__input')
          $conditionals = await $component.$$('.city-checkboxes__conditional')
        })
        ...
        it('has no ARIA attributes applied', async () => {
          const $inputsWithAriaExpanded = await $component.$$(
            '.city-checkboxes__input[aria-expanded]'
          )
          const $inputsWithAriaControls = await $component.$$(
            '.city-checkboxes__input[aria-controls]'
          )
  ...
  describe('with a "None" checkbox and conditional reveals', () => {
    describe('when JavaScript is available', () => {
      let $component
      let $inputs
  ...
  describe('with multiple groups and a "None" checkbox and conditional reveals', () => {
    describe('when JavaScript is available', () => {
      ...
      beforeEach(async () => {
        ...
        $inputsPrimary = await page.$$(
          '.city-checkboxes__input[id^="colour-primary"]'
        )
        $inputsSecondary = await page.$$(
          '.city-checkboxes__input[id^="colour-secondary"]'
        )
        $inputsOther = await page.$$(
          '.city-checkboxes__input[id^="colour-other"]'
        )
      })
          ...
          await expect(
            render(page, 'checkboxes', examples.default, {
              beforeInitialisation($module) {
                ...
                $module.outerHTML = `<svg data-module="city-checkboxes"></svg>`
              }
            })
          ).rejects.toMatchObject({
            cause: {
        ...
        it('throws when the input list is empty', async () => {
          await expect(
            render(page, 'checkboxes', examples.default, {
              ...
              context: {
                selector: '.city-checkboxes__item'
              }
            })
          ).rejects.toMatchObject({
            cause: {
        ...
        it('throws when a conditional target element is not found', async () => {
          await expect(
            render(page, 'checkboxes', examples['with conditional items'], {
              ...
              context: {
                selector: '.city-checkboxes__conditional'
              }
            })
          ).rejects.toMatchObject({
            cause: {
