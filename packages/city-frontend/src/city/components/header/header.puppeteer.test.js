...
describe('Header navigation', () => {
  ...
  describe('when JavaScript is unavailable or fails', () => {
    ...
    it('does not show the mobile menu button', async () => {
      const buttonDisplay = await page.$eval('.city-js-header-toggle', (el) =>
  ...
  describe('when JavaScript is available', () => {
    ...
    describe('on page load', () => {
      ...
      it('reveals the menu button', async () => {
        const hidden = await page.$eval('.city-js-header-toggle', (el) =>
      ...
      it('hides the menu via the hidden attribute', async () => {
        const hidden = await page.$eval('.city-header__navigation-list', (el) =>
          el.hasAttribute('hidden')
        )
    ...
    describe('when menu button is pressed', () => {
      beforeAll(async () => {
        ...
        await page.waitForSelector('.city-js-header-toggle')
        await page.click('.city-js-header-toggle')
      })
      ...
      it("throws when the toggle's aria-control attribute is missing", async () => {
        await expect(
          render(page, 'header', examples['with navigation'], {
            ...
            context: {
              selector: '.city-js-header-toggle'
            }
          })
        ).rejects.toMatchObject({
          cause: {
            name: 'ElementError',
            message:
              'Header: Navigation button (`<button class="city-js-header-toggle">`) attribute (`aria-controls`) not found'
          }
        })
      })
