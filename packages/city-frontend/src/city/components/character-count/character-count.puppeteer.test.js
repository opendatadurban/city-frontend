...
describe('Character count', () => {
  ...
  describe('when JavaScript is unavailable or fails', () => {
    ...
    it('shows the textarea description', async () => {
      ...
      const message = await page.$eval('.city-character-count__message', (el) =>
        el.innerHTML.trim()
      )
  ...
  describe('when JavaScript is available', () => {
    describe('on page load', () => {
      ...
      it('injects the visual counter', async () => {
        const message = (await page.$('.city-character-count__status')) !== null
        expect(message).toBeTruthy()
      })

      it('injects the screen reader counter', async () => {
        const srMessage =
          (await page.$('.city-character-count__sr-status')) !== null
        expect(srMessage).toBeTruthy()
      })

      it('hides the textarea description', async () => {
        const messageClasses = await page.$eval(
          '.city-character-count__message',
          (el) => el.className
        )
        expect(messageClasses).toContain('city-visually-hidden')
      })
    })
    ...
    describe('when counting characters', () => {
      ...
      it('counts down to the character limit', async () => {
        ...
        await page.type('.city-js-character-count', 'A', {
          delay: 50
        })
      ...
      it('uses the singular when there is only one character remaining', async () => {
        ...
        await page.type('.city-js-character-count', 'A'.repeat(9), {
          delay: 50
        })
      ...
      describe('when the character limit is exceeded', () => {
        beforeAll(async () => {
          ...
          await page.type('.city-js-character-count', 'A'.repeat(11), {
            delay: 50
          })
        })
        ...
        it('adds error styles to the textarea', async () => {
          const textareaClasses = await page.$eval(
            '.city-js-character-count',
            (el) => el.className
          )
          expect(textareaClasses).toContain('city-textarea--error')
        })
        ...
        it('adds error styles to the count message', async () => {
          ...
          expect(messageClasses).toContain('city-error-message')
        })
      })
      ...
      describe('when a threshold is set', () => {
        ...
        it('becomes visible once the threshold is reached', async () => {
          await page.type('.city-js-character-count', 'A'.repeat(8), {
            delay: 50
          })
      ...
      describe('when a maxlength attribute is specified on the textarea', () => {
        ...
        it('should not have a maxlength attribute once the JS has run', async () => {
          const textareaMaxLength = await page.$eval('.city-textarea', (el) =>
    ...
    describe('when counting words', () => {
      ...
      it('counts down to the word limit', async () => {
        ...
        await page.type('.city-js-character-count', 'Hello world', {
          delay: 50
        })
      ...
      it('uses the singular when there is only one word remaining', async () => {
        ...
        await page.type('.city-js-character-count', 'Hello '.repeat(9), {
          delay: 50
        })
      ...
      describe('when the word limit is exceeded', () => {
        beforeAll(async () => {
          ...
          await page.type('.city-js-character-count', 'Hello '.repeat(11), {
            delay: 50
          })
        })
        ...
        it('uses the plural when the limit is exceeded by 2 or more', async () => {
          await page.type('.city-js-character-count', 'World', {
            delay: 50
          })
        ...
        await expect(
          render(page, 'character-count', examples.default, {
            beforeInitialisation($module) {
              ...
              $module.outerHTML = `<svg data-module="city-character-count"></svg>`
            }
          })
        ).rejects.toMatchObject({
          cause: {
      ...
      it('throws when the textarea is missing', async () => {
        await expect(
          render(page, 'character-count', examples.default, {
            ...
            context: {
              selector: '.city-js-character-count'
            }
          })
        ).rejects.toMatchObject({
          cause: {
            name: 'ElementError',
            message:
              'Character count: Form field (`.city-js-character-count`) not found'
          }
        })
      })
      ...
      it('throws when the textarea is not the right type', async () => {
        await expect(
          render(page, 'character-count', examples.default, {
            beforeInitialisation($module, { selector }) {
              ...
              $module.querySelector(selector).outerHTML =
                '<div class="city-js-character-count"></div>'
            },
            context: {
              selector: '.city-js-character-count'
            }
          ...
          cause: {
            name: 'ElementError',
            message:
              'Character count: Form field (`.city-js-character-count`) is not of type HTMLTextareaElement or HTMLInputElement'
          }
        })
      })
  ...
  describe('in mismatched locale', () => {
    it('does not error', async () => {
      ...
      await page.type('.city-js-character-count', 'A'.repeat(10), {
        delay: 50
      })
