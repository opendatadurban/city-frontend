...
describe('dist/', () => {
  ...
  beforeAll(async () => {
    listingSourceAssets = await getListing('**/*', {
      cwd: join(paths.package, 'src/city/assets')
    })
  ...
  describe('city-frontend-[version].min.css', () => {
    let filename
    let stylesheet

    beforeAll(async () => {
      filename = `city-frontend-${pkg.version}.min.css`
    ...
    it('should contain CSS custom properties', () => {
      ...
      expect(stylesheet).toContain(`--city-frontend-version:"${pkg.version}"`)

      // Breakpoints for `window.matchMedia()` in Header, Tabs components
      expect(stylesheet).toContain('--city-frontend-breakpoint-mobile:20rem')
      expect(stylesheet).toContain(
        '--city-frontend-breakpoint-tablet:40.0625rem'
      )
      expect(stylesheet).toContain(
        '--city-frontend-breakpoint-desktop:48.0625rem'
      )
    })
  })

  describe('city-frontend-[version].min.css.map', () => {
    let filename
    let sourcemap

    beforeAll(async () => {
      filename = `city-frontend-${pkg.version}.min.css.map`
    ...
    it('should contain relative paths to sources', () => {
      expect(sourcemap.sources).toContain(
        '../packages/city-frontend/src/city/all.scss'
      )
      expect(sourcemap.sources).toContain(
        '../packages/city-frontend/src/city/core/_city-frontend-properties.scss'
      )
    })
  })

  describe('city-frontend-[version].min.js', () => {
    let filename
    let javascript

    beforeAll(async () => {
      filename = `city-frontend-${pkg.version}.min.js`
  ...
  describe('city-frontend-[version].min.js.map', () => {
    let filename
    let sourcemap

    beforeAll(async () => {
      filename = `city-frontend-${pkg.version}.min.js.map`
      sourcemap = JSON.parse(
        await readFile(join(paths.root, `dist/${filename}`), 'utf8')
      )
    })
    ...
    it('should contain relative paths to sources', () => {
      expect(sourcemap.sources).toContain(
        '../packages/city-frontend/src/city/init.mjs'
      )
      expect(sourcemap.sources).toContain(
        '../packages/city-frontend/src/city/common/city-frontend-version.mjs'
      )
    })
  })
