...
import { paths, pkg } from '@city-frontend/config'
import { compileSassFile } from '@city-frontend/helpers/tests'
import {
  getComponentNames,
  getComponentNamesFiltered
} from '@city-frontend/lib/components'
import { filterPath, getListing, mapPathTo } from '@city-frontend/lib/files'
import { componentNameToClassName } from '@city-frontend/lib/names'
import { outdent } from 'outdent'

describe('packages/city-frontend/dist/', () => {
  let listingPackage
  let listingSource
  let listingDist
  ...
  beforeAll(async () => {
    ...
    componentsFilesSource = await getListing('**/*', {
      cwd: join(paths.package, 'src/city/components')
    })

    componentsFilesDist = await getListing('**/*', {
      cwd: join(paths.package, 'dist/city/components')
    })
  ...
  it('should contain the expected files', async () => {
    ...
    const listingExpected = listingSource
      ...
      .flatMap(mapPathTo(['**/city-prototype-kit.config.mjs'], () => []))
      ...
      .flatMap(
        mapPathTo(['**/city/all.mjs'], ({ dir: requirePath }) => [
          join(requirePath, 'all.mjs'),

          // ES module bundles for browsers, minified
          join(requirePath, 'city-frontend.min.js'), // avoid .mjs extension MIME issues
          join(requirePath, 'city-frontend.min.js.map') // with source map
        ])
      )

      // Only sass package entry is compiled to minified CSS bundle
      .flatMap(
        mapPathTo(['**/city/all.scss'], ({ dir: requirePath }) => [
          join(requirePath, 'all.scss'),

          // CSS bundle, minified
          join(requirePath, 'city-frontend.min.css'),
          join(requirePath, 'city-frontend.min.css.map') // with source map
        ])
      )
    ...
    expect(listingPackage).toEqual([
      'README.md',
      'babel.config.js',
      'city-prototype-kit.config.json',
      'gulpfile.mjs',
      'package.json',
  ...
  describe('Sass stylesheets', () => {
    describe('all.scss', () => {
      it('should compile without throwing an exception', async () => {
        const file = join(paths.package, 'dist/city/all.scss')
  ...
  describe('ECMAScript (ES) modules', () => {
    describe('all.mjs', () => {
      it('should export each module', async () => {
        const contents = await readFile(
          join(paths.package, 'dist/city/all.mjs'),
          'utf8'
        )
    ...
    describe('common/city-frontend-version.mjs', () => {
      it('should have correct version number', async () => {
        const contents = await readFile(
          join(paths.package, 'dist/city/common/city-frontend-version.mjs'),
          'utf8'
        )
  ...
  describe('Universal Module Definition (UMD)', () => {
    describe('all.bundle.js', () => {
      it('should export each module', async () => {
        const contents = await readFile(
          join(paths.package, 'dist/city/all.bundle.js'),
          'utf8'
        )

        // Look for AMD module definition for 'cityFrontend'
        expect(contents).toContain(
          "(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.cityFrontend = {}));"
        )
    ...
    it('should have macro-options.json that contains JSON', () => {
      const componentTasks = componentNames.map(async (componentName) => {
        ...
        const options = JSON.parse(
          await readFile(
            join(paths.package, 'dist/city/components', optionsPath),
            'utf8'
          )
        )
      ...
      const componentTasks = componentNamesWithJavaScript.map(
        async (componentName) => {
          ...
          const moduleTextESM = await readFile(
            join(paths.package, 'dist/city/components', modulePathESM),
            'utf8'
          )
          const moduleTextUMD = await readFile(
            join(paths.package, 'dist/city/components', modulePathUMD),
            'utf8'
          )
    ...
    it('should have fixtures.json that contains JSON', () => {
      const componentTasks = componentNames.map(async (componentName) => {
        ...
        const { component, fixtures } = JSON.parse(
          await readFile(
            join(paths.package, 'dist/city/components', fixturesPath),
            'utf8'
          )
        )
