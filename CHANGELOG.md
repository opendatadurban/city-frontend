...
#### Use tabular numbers with the `city-font-tabular-numbers` mixin
...
This change was introduced in [pull request #3559: Remove IE8-related Sass and CSS build tasks](https://github.com/alphagov/city-frontend/pull/3559).
...
This change was introduced in [pull request #3949: Simplify font family settings](https://github.com/alphagov/city-frontend/pull/3949).
...
This change was introduced in [pull request #3641: Remove fallback GOV.UK logo for IE8](https://github.com/alphagov/city-frontend/pull/3641).
...
We've removed the override classes starting with `city-!-margin-static` and `city-!-padding-static`, which were deprecated in [GOV.UK Frontend v4.3.1](https://github.com/alphagov/city-frontend/releases/tag/v4.3.1).
...
This was added in [pull request #3344: Adding optional ‘id’ attribute to button component](https://github.com/alphagov/city-frontend/pull/3344).
...
This was added in [pull request #2702: Allow localisation of content licence and copyright notices in Footer](https://github.com/alphagov/city-frontend/pull/2702).
...
If you’re not using Nunjucks macros, update your hint HTML to use a `<div>` rather than a `<span>`. For example: `<div id="input-hint" class="city-hint"></div>` instead of `<span id="input-hint" class="city-hint"></span>`.

This change was introduced in [pull request #2437: Remove `display:block` on hint component](https://github.com/alphagov/city-frontend/pull/2437).
...
If you're not using Nunjucks macros, add a `data-module="city-skip-link"` attribute to the component HTML. For example:
...
This change was introduced in [pull request #2450: Set focus to skip link target to improve screen reader announcements](https://github.com/alphagov/city-frontend/pull/2450).
...
If you’re not using Nunjucks macros, swap the `<span class="city-error-message">` for a `<p class="city-error-message">`.

This change was introduced in [pull request #2452: Update error messages to use paragraph tags instead of spans](https://github.com/alphagov/city-frontend/pull/2452).
...
This change was introduced in [pull request #2370: Prevent issues with conditionally revealed content when content `id` includes CSS syntax characters](https://github.com/alphagov/city-frontend/pull/2370).
...
This change was introduced in [pull request #2408: Prevent issues with character count when textarea `id` includes CSS syntax characters](https://github.com/alphagov/city-frontend/pull/2408).
...
`window.cityFrontend.[ComponentName]` instead of `window.cityFrontend`.
...
If you're importing JavaScript modules individually, you should check any references to `window.cityFrontend` in your code and update them to point to the correct component, `window.cityFrontend.[ComponentName]`. You can now remove any workarounds you may have implemented.

This change was introduced in [pull request #2426: Rename exported JavaScript modules to include component name](https://github.com/alphagov/city-frontend/pull/2426). Thanks to [Alex Jurubita](https://github.com/alex-ju) for reporting this issue.

#### Import 'base' before importing Sass files from `core` or `overrides` layers

If you import individual files from the `core` or `overrides` layers, you might see the error `Undefined mixin 'city-exports'` or `no mixin named city-exports` when compiling your Sass.

To fix the error, import `node_modules/city-frontend/city/base` first. For example:
...
This change was introduced in [pull request #2455: Remove 'base' import from files in `core` and `overrides` layers](https://github.com/alphagov/city-frontend/pull/2455).
...
To fix this error, replace any imports of `node_modules/city-frontend/city/core/template` with `node_modules/city-frontend/city/objects/template`.

This change was introduced in [pull request #2463: Move template styles from `core` to `objects` layer](https://github.com/alphagov/city-frontend/pull/2463).
...
If you're not using the Nunjucks macros, check your footer displays as expected and use the standard Design System grid classes on the `city-footer__section` element to set the width. For example:

```html
<div class="city-footer__section city-grid-column-two-thirds">...</div>
```

This change was introduced in [pull request #2462: Fix footer alignment with grid classes and add `width` macro option](https://github.com/alphagov/city-frontend/pull/2462).

#### Remove deprecated `$city-border-width-form-element-error` variable

In [GOV.UK Frontend v3.8.0](https://github.com/alphagov/city-frontend/releases/tag/v3.8.0), we made the border width of form elements in their error state the same as for form elements in their normal state and deprecated the `$city-border-width-form-element-error` variable.
...
We've now removed `$city-border-width-form-element-error` completely.

If you’re referencing `$city-border-width-form-element-error` in your own Sass, you must remove it. If you're also defining your own error state, you only need to change the border colour.

You should avoid overriding the border width. For example, replace `border: $city-border-width-form-element-error solid $city-error-colour;` with `border-color: $city-error-colour;`.

You should also remove any override of `$city-border-width-form-element-error` from your Sass. This override no longer does anything.

This change was introduced in [pull request #1963: Remove deprecated `$city-border-width-form-element-error` variable](https://github.com/alphagov/city-frontend/pull/1963).

#### Remove deprecated `city-main-wrapper` and `city-main-wrapper--l` mixins

We've removed the `city-main-wrapper` and `city-main-wrapper--l` mixins we deprecated in [GOV.UK Frontend v3.0.0](https://github.com/alphagov/city-frontend/releases/tag/v3.0.0).
...
This change was introduced in [pull request #2385: Remove deprecated `city-main-wrapper` and `city-main-wrapper--l` mixins](https://github.com/alphagov/city-frontend/pull/2385).

#### Remove calls to deprecated `iff` Sass function

We've removed the `iff` function we deprecated in [GOV.UK Frontend version 3.6.0](https://github.com/alphagov/city-frontend/releases/tag/v3.6.0).
...
This change was introduced in [pull request #2409: Remove deprecated `iff` Sass function](https://github.com/alphagov/city-frontend/pull/2409).

#### Remove deprecated `city-tag--inactive` class

We've removed the `city-tag--inactive` class we deprecated in [GOV.UK Frontend v3.6.0](https://github.com/alphagov/city-frontend/releases/tag/v3.6.0).

Replace any use of this class with the `city-tag--grey` class.

This change was introduced in [pull request #2417: Remove deprecated `city-tag--inactive class`](https://github.com/alphagov/city-frontend/pull/2417).
...
There are some known issues with inputs of `type="number"`. [Read our research into the issues with `type="number"`.](https://github.com/alphagov/city-frontend/issues/1449#issuecomment-503186819)
...
This change was introduced in [pull request #1704: Update date input to use `type=text` and `inputmode=numeric`](https://github.com/alphagov/city-frontend/pull/1704).
...
This change was introduced in [pull request #1724: Fix fallback logo so Chrome will not flag it to screen readers](https://github.com/alphagov/city-frontend/pull/1724).
...
This change was introduced in [pull request #2192: Add `data-nosnippet` to prevent cookie banner text appearing in Google Search snippets](https://github.com/alphagov/city-frontend/pull/2192).

### Fixes

We’ve made fixes to GOV.UK Frontend in the following pull requests:

- [#2255: Prevent conditionally revealed questions getting out of sync when multiple sets of radios and checkboxes contain inputs with the same name](https://github.com/alphagov/city-frontend/pull/2255)
- [#2323: Avoid invalid nesting of `<span>` within a `<dd>` in summary list](https://github.com/alphagov/city-frontend/pull/2323)
- [#2370: Prevent issues with conditionally revealed content when content `id` includes CSS syntax characters](https://github.com/alphagov/city-frontend/pull/2370)
- [#2408: Prevent issues with character count when textarea `id` includes CSS syntax characters](https://github.com/alphagov/city-frontend/pull/2408)
- [#2426: Rename exported JavaScript modules to include component name](https://github.com/alphagov/city-frontend/pull/2426)
- [#2434: Add brand colour for Department for Levelling Up, Housing and Communities (DLUHC)](https://github.com/alphagov/city-frontend/pull/2434) - thanks to [Bruce Bolt](https://github.com/brucebolt) for contributing this change
- [#2447: Remove bottom margin from navigation on tablets](https://github.com/alphagov/city-frontend/pull/2447)
- [#2448: Remove the border from the last item in the collapsible navigation menu in the header](https://github.com/alphagov/city-frontend/pull/2448)
...
You can now use the `city-!-text-align-left`, `city-!-text-align-centre` and `city-!-text-align-right` CSS classes to set text alignment on elements.

This was added in [pull request #2339: Add `text-align` override classes](https://github.com/alphagov/city-frontend/pull/2339). Thanks to [Ed Horsford](https://github.com/edwardhorsford) for reporting this issue.

#### Define negative spacing using the `city-spacing` function

You can now pass the negative equivalent of a point from the spacing scale to the `city-spacing` function to get negative spacing.

For example, `city-spacing(1)` returns `5px`, and `city-spacing(-1)` returns `-5px`.

This was added in [pull request #2348: Allow `city-spacing` to output negative spacing](https://github.com/alphagov/city-frontend/pull/2348). Thanks to [Chris Hill-Scott](https://github.com/quis) for reporting this issue.

### Fixes

- [#2347: Prevent panel text overflowing when zoomed in on mobile](https://github.com/alphagov/city-frontend/pull/2347) - thanks to [Phil Sherry](https://github.com/philsherry) for reporting this issue.
...
- [#2264: Improve focus state for radio and checkbox controls in forced colors mode (for example, Windows High Contrast Mode)](https://github.com/alphagov/city-frontend/pull/2264) – thanks to [@adamliptrot-oc](https://github.com/adamliptrot-oc) for reporting this issue
- [#2265: Do not remove focus outline from disabled link buttons in forced colors mode](https://github.com/alphagov/city-frontend/pull/2265)
- [#2273: Fix invisible footer on Open Government Licence logo in forced colors mode](https://github.com/alphagov/city-frontend/pull/2273) – thanks to [@oscarduignan](https://github.com/oscarduignan) for reporting this issue
- [#2277: Fix invisible start button chevron in forced colors mode](https://github.com/alphagov/city-frontend/pull/2277)
- [#2290: Improve support for Internet Explorer 11 with Windows High Contrast Mode](https://github.com/alphagov/city-frontend/pull/2290)
- [#2306: Add `max-width` to file upload component](https://github.com/alphagov/city-frontend/pull/2306)
- [#2312: Remove `padding-right` from details component](https://github.com/alphagov/city-frontend/pull/2312)
...
This was added in [pull request #2151: Add 'None of the above' and 'or' divider to checkboxes](https://github.com/alphagov/city-frontend/pull/2151). Thanks to [@frankieroberto](https://github.com/frankieroberto) for contributing this improvement.
...
This was added in [pull request #2245: Allow the CSP `nonce` attribute to be set on the inline script in the page template](https://github.com/alphagov/city-frontend/pull/2245). Thanks to [@natcarey](https://github.com/natcarey) for contributing this issue and its solution.

### Fixes

We've made the following fixes in [pull request #2229: Change approach to fallback PNG in the header to fix blank data URI from triggering Content Security Policy error](https://github.com/alphagov/city-frontend/pull/2229):
...
- [#2228: Fix display of checkboxes in Internet Explorer 8](https://github.com/alphagov/city-frontend/pull/2228)
- [#2237: Fix GOV.UK logo disappearing on light background in Windows High Contrast Mode](https://github.com/alphagov/city-frontend/pull/2237)
- [#2251: Disable ink skipping for underlines in hover state](https://github.com/alphagov/city-frontend/pull/2251)
...
- want to avoid opting in if you have links within a [multiple-column layout](https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Multiple-column_Layout) in your CSS - there is [a Chromium bug that affects such links](https://github.com/alphagov/city-frontend/issues/2204)

To enable this feature, set `$city-new-link-styles` to `true` before you import GOV.UK Frontend in your Sass:
...
This was added in [pull request #2183: Updates to link styles and link hover states](https://github.com/alphagov/city-frontend/pull/2183).

#### Style links to remove underlines

You can now remove underlines from links by using either the:

- [`city-link-style-no-underline` mixin](http://frontend.design-system.service.gov.uk/sass-api-reference/#city-link-style-no-underline) in your Sass, or
- [`city-link--no-underline` class](https://design-system.service.gov.uk/styles/typography/#links-without-underlines) in your HTML

An underline still appears when the user hovers their cursor over the link.

This was added in [pull request #2214: Add no-underline mixin and modifier class](https://github.com/alphagov/city-frontend/pull/2214).

#### Style links on dark backgrounds

You can now style links on dark backgrounds to use white text colour by using either the:

- [`city-link-style-inverse` mixin](http://frontend.design-system.service.gov.uk/sass-api-reference/#city-link-style-inverse) in your Sass, or
- [`city-link--inverse` class](https://design-system.service.gov.uk/styles/typography/#links-on-dark-backgrounds) in your HTML

This was added in [pull request #2213: Add inverse link mixin and modifier class](https://github.com/alphagov/city-frontend/pull/2213).
...
This was added in [pull request #2164: Enable cookie banner to set link styled as a button](https://github.com/alphagov/city-frontend/pull/2164).

### Fixes

- [#2132: Improve vertical alignment of phase banner tag on mobile devices](https://github.com/alphagov/city-frontend/pull/2132) – thanks to [@matthewmascord](https://github.com/matthewmascord) for contributing this issue.
- [#2157: Use pointer cursor for 'Menu' button in header](https://github.com/alphagov/city-frontend/pull/2157) – thanks to [@MalcolmVonMoJ](https://github.com/MalcolmVonMoJ) for contributing this issue.
- [#2171: Fix padding on GOV.UK logo affecting hover and focus states](https://github.com/alphagov/city-frontend/pull/2171)
- [#2186: Fix display of warning text in Edge when Windows High Contrast Mode is enabled](https://github.com/alphagov/city-frontend/pull/2186)
- [#2192: Add data-nosnippet to prevent cookie banner text appearing in Google Search snippets](https://github.com/alphagov/city-frontend/pull/2192)
- [#2201: Set -webkit-appearance: button on file upload so text is aligned in Safari](https://github.com/alphagov/city-frontend/pull/2201)
- [#2205: Stop file upload component 'jumping' on focus](https://github.com/alphagov/city-frontend/pull/2205)
- [#2212: Add underline to crown copyright link in footer](https://github.com/alphagov/city-frontend/pull/2212)
...
This was added in [pull request #2131: Add cookie banner component and button groups](https://github.com/alphagov/city-frontend/pull/2131).
...
This was added in [pull request #2114: Add button groups for use in cookie banner](https://github.com/alphagov/city-frontend/pull/2114).

#### Change the style of table captions

You can now make table captions larger or smaller by applying the `city-table__caption--xl`, `city-table__caption--l`, `city-table__caption--m` or `city-table__caption--s` classes.

This was added in [pull request #2048: Add modifiers for table captions](https://github.com/alphagov/city-frontend/pull/2048).

### Fixes

We’ve made fixes to GOV.UK Frontend in the following pull requests:

- [#2045: Stop same-site cookies from being wiped when printing in Internet Explorer 11](https://github.com/alphagov/city-frontend/pull/2045) – thanks to [@gunjam](https://github.com/gunjam) for contributing this issue.
- [#2080: Fix JavaScript error when character count ID starts with a number](https://github.com/alphagov/city-frontend/pull/2080) - thanks to [@josef-vlach](https://github.com/josef-vlach) for reporting this issue.
- [#2092: Use tabular numbers for character count message](https://github.com/alphagov/city-frontend/pull/2092) - thanks to [@quis](https://github.com/quis) for contributing this issue.
- [#2093: Only output space after breadcrumbs class if there’s an additional class](https://github.com/alphagov/city-frontend/pull/2093) – thanks to [@frankieroberto](https://github.com/frankieroberto) for contributing this issue.
- [#2133: Remove user-agent default 2px horizontal margins on buttons in Safari](https://github.com/alphagov/city-frontend/pull/2133)
...
- [#2031: Include hidden status of the component examples in published fixtures](https://github.com/alphagov/city-frontend/pull/2031) – thanks to [@andymantell](https://github.com/andymantell) for contributing this
- [#2043: Fixes for a couple of component examples / fixtures](https://github.com/alphagov/city-frontend/pull/2043) – thanks to [@andymantell](https://github.com/andymantell) for contributing this as well!
- [#2071: Use non-bool type strings for attributes in test fixtures](https://github.com/alphagov/city-frontend/pull/2071) – thanks to [@matthew-shaw](https://github.com/matthew-shaw) for contributing this
...
- [#2035: Fix hairline gap between notification banner header and outer border on high resolution screens in Chrome/Edge (Blink)](https://github.com/alphagov/city-frontend/pull/2035)
- [#2036: Explicitly set the text colour for notification banner content, rather than using the user-agent default text colour](https://github.com/alphagov/city-frontend/pull/2036)
- [#2042: Wrap notification banner content at 2/3 grid width](https://github.com/alphagov/city-frontend/pull/2042)
...
This was added in [pull request #1935: Create notification banner component](https://github.com/alphagov/city-frontend/issues/1935).
...
This was added in [pull request #1975: Enable custom inputmode for date input component](https://github.com/alphagov/city-frontend/pull/1975). Thanks to [@foaly-nr1](https://github.com/foaly-nr1) for contributing this issue.
...
- [#1967: Fix navigation links in the header not being announced by screen readers](https://github.com/alphagov/city-frontend/pull/1967)
...
This was added in [pull request #1816: Add input prefix and suffix](https://github.com/alphagov/city-frontend/pull/1816). Thanks to [@simonwhatley](https://github.com/simonwhatley) and the GOV.UK Coronavirus Services Team.
...
This was added in [pull request #1925: Generate fixtures.json files for components on build:package](https://github.com/alphagov/city-frontend/pull/1925). Thanks to everyone who fed back on [our test fixtures proposal](https://github.com/alphagov/city-frontend/issues/1830#issuecomment-665075842).
...
- [#1905: Set navigation and mobile menu labels of the header component with new options](https://github.com/alphagov/city-frontend/pull/1905)
- [#1943: Change header menu button label](https://github.com/alphagov/city-frontend/pull/1943) - thanks to [@domoscargin](https://github.com/domoscargin) for raising this issue
...
This was added in [pull request #1921: Make it possible to exclude link from header navigation item](https://github.com/alphagov/city-frontend/pull/1921).

### Fixes

We’ve made fixes to GOV.UK Frontend in the following pull requests:

- [#1918: Add new brand colour for FCDO](https://github.com/alphagov/city-frontend/pull/1918) - thanks to [@deborahchua](https://github.com/deborahchua) and [@beccapearce](https://github.com/beccapearce) for contributing this
- [#1942: Set aria-expanded and aria-hidden attributes on header menu button and menu when page loads](https://github.com/alphagov/city-frontend/pull/1942)
- [#1947 Add print styles for the panel component](https://github.com/alphagov/city-frontend/pull/1947)
...
- [#1912: Fix character count shrinking as you go over limit](https://github.com/alphagov/city-frontend/pull/1912)
...
`$city-secondary-text-colour` and `city-colour("dark-grey")` are now darker so users can more clearly read hint text that uses the colour.
...
This was added in [pull request #1827: Make dark grey darker](https://github.com/alphagov/city-frontend/pull/1827).
...
This was added in [pull request #1870: Reduce border width of form inputs in the error state](https://github.com/alphagov/city-frontend/pull/1870).
...
- [#1859: Add spellcheck option](https://github.com/alphagov/city-frontend/pull/1859)
- [#1869: Add missing spellcheck param to character count](https://github.com/alphagov/city-frontend/pull/1869)

### Deprecated features

#### $city-border-width-form-element-error

From GOV.UK Frontend v4.0.0, you'll no longer be able to reference the [`$city-border-width-form-element-error`](https://frontend.design-system.service.gov.uk/sass-api-reference/#city-border-width-form-element-error) Sass setting.

Change any references to `$city-border-width-form-element-error` in your Sass code so they reference `$city-border-width-form-element` instead.

This was changed in [pull request #1870: Reduce border width of form inputs in the error state](https://github.com/alphagov/city-frontend/pull/1870).

### Fixes

We’ve made fixes to GOV.UK Frontend in the following pull requests:

- [#1838: Correctly camel case SVG attributes in the header and footer](https://github.com/alphagov/city-frontend/pull/1838)
- [#1842: Preserve the state of conditional reveals when navigating 'back' in the browser](https://github.com/alphagov/city-frontend/pull/1842)
- [#1848: Preserve the state of the character count when navigating 'back' in the browser](https://github.com/alphagov/city-frontend/pull/1848)
- [#1855: Hint component can render block-level elements as valid HTML](https://github.com/alphagov/city-frontend/pull/1855)
- [#1861: Fix the display of checkboxes when border-box box sizing is applied globally](https://github.com/alphagov/city-frontend/pull/1861)
- [#1862: Fix display of warning text icon when border-box box sizing is applied globally #1862](https://github.com/alphagov/city-frontend/pull/1862)
- [#1879: Explicitly set outline-offset to remove 1px transparent border in chrome v84](https://github.com/alphagov/city-frontend/pull/1879)
...
If a [list](https://design-system.service.gov.uk/styles/typography/#lists) is hard to read because the items run across multiple lines, you can now [add extra spacing between list items](http://design-system.service.gov.uk/styles/typography/#adding-extra-spacing-between-list-items) using the new `city-list--spaced` class.

This was added in [pull request #1775: Add list--spaced modifier](https://github.com/alphagov/city-frontend/pull/1775). Thanks to [@frankieroberto](https://github.com/frankieroberto) for raising this issue.
...
This was added in [pull request #1819: Add the ability to specify HTML for a navigation item](https://github.com/alphagov/city-frontend/pull/1819). Thanks to [@adamsilver](https://github.com/adamsilver).

#### Import settings, tools and helpers CSS in one line

You can now import `node_modules/city-frontend/city/base`, instead of importing `settings`, `helpers` and `tools` separately.
...
If you already import `node_modules/city-frontend/city/all` in your Sass file, you do not need to do anything. Sass will automatically compile faster.
...
This was added in [pull request #1804: Allow components to be imported without dependencies](https://github.com/alphagov/city-frontend/pull/1804). Thanks to [@kevindew](https://github.com/kevindew) for raising this issue.
...
This was added in [pull request #1754: Add collapseOnMobile breadcrumbs flag](https://github.com/alphagov/city-frontend/pull/1754). Thanks to [@vanitabarrett](https://github.com/vanitabarrett) and [@miaallers](https://github.com/miaallers).
...
This was added in [pull request #1753: Make back link arrow consistent with breadcrumb component](https://github.com/alphagov/city-frontend/pull/1753). Thanks to [@vanitabarrett](https://github.com/vanitabarrett) and [@miaallers](https://github.com/miaallers).

### Deprecated features

#### Importing from the `core` and `overrides` layers without `base`

If you import specific files from the `core` or `overrides` layers, you’ll now see a deprecation warning when compiling Sass if you do not import `node_modules/city-frontend/city/base` first.

To fix the warning, import `node_modules/city-frontend/city/base` first. For example:
...
If you do not import `node_modules/city-frontend/city/base` first, your service will no longer work from GOV.UK Frontend v4.0.0.

This was added in [pull request #1807: Warn if importing core, overrides without dependencies](https://github.com/alphagov/city-frontend/pull/1807).

### Fixes

We’ve made fixes to GOV.UK Frontend in the following pull requests:

- [#1778: Fix accordion underline hover state being removed when hovering plus/minus symbol](https://github.com/alphagov/city-frontend/pull/1778)
- [#1765: Import textarea from character count](https://github.com/alphagov/city-frontend/pull/1765)
- [#1796: Standardise accordion section headings font size (reduce height of section headings on mobile)](https://github.com/alphagov/city-frontend/pull/1796)
...
This also means you should replace the `.city-tag--inactive` class with the `.city-tag--grey` class. `.city-tag--inactive` is now deprecated, and it will be removed in a future release.

[Pull request #1711: Additional Tag modifier classes for different colours](https://github.com/alphagov/city-frontend/pull/1711).

#### Hide elements when users print a page

You can now hide elements when users print a page, using the new `city-!-display-none-print` class.

[Pull request #1723: Add display override for hiding content when printing](https://github.com/alphagov/city-frontend/pull/1723).
...
[Pull request #1742: Deprecate iff function](https://github.com/alphagov/city-frontend/pull/1742).

### Fixes

- [Pull request #1724: Fix fallback logo being detected by Google Chrome's image description feature](https://github.com/alphagov/city-frontend/pull/1724).
- [Pull request #1745: Update vendor polyfills to match upstream](https://github.com/alphagov/city-frontend/pull/1745).
- [Pull request #1746: Use generic div element for tabspanel](https://github.com/alphagov/city-frontend/pull/1746).
...
- [Pull request #1650: Make Character Count use hint component for message and allow custom classes to be added](https://github.com/alphagov/city-frontend/pull/1650).

### Fixes

- [Pull request #1704: Update the date input component to use `input type=text inputmode=numeric`](https://github.com/alphagov/city-frontend/pull/1704).
- [Pull request #1690: Don't unneccesarily self-close tags](https://github.com/alphagov/city-frontend/pull/1690).
- [Pull request #1678: Fix tabs component throwing JavaScript errors in Internet Explorer 8](https://github.com/alphagov/city-frontend/pull/1678).
- [Pull request #1676: Fix skip link component focus style with global styles enabled](https://github.com/alphagov/city-frontend/pull/1676).
- [Pull request #1672: Ensure footer links look clickable](https://github.com/alphagov/city-frontend/pull/1672).
- [Pull request #1670: Make width-container margins more targetted to avoid specificity issues](https://github.com/alphagov/city-frontend/pull/1670).
- [Pull request #1655: Ensure components use public `city-media-query` mixin](https://github.com/alphagov/city-frontend/pull/1655).
- [Pull request #1648: Update checkboxes and radio buttons to include item hint classes on item hint](https://github.com/alphagov/city-frontend/pull/1648).
- [Pull request #1638: Check component item arrays are not empty before outputting markup](https://github.com/alphagov/city-frontend/pull/1638).
...
[Pull request #1626: Allow creating custom width containers and using them with template](https://github.com/alphagov/city-frontend/pull/1626).

#### Set a custom width for wrappers

If you're using Sass, you can now use the `city-width-container` mixin to create a custom wrapper class with a specific width. You can then add that class to the following wrappers to override the width of the `city-width-container` class:
...
To create your custom wrapper class, include the `city-width-container` mixin. Pass in the width in pixels.
...
Use the `$city-page-width` Sass variable instead if all your pages are the same width.
...
- [Pull request #1623: Allow attributes to be set on template <body>](https://github.com/alphagov/city-frontend/pull/1623).

### Fixes

- [Pull request #1594: Refactor handling of count message in character count JavaScript](https://github.com/alphagov/city-frontend/pull/1594).
- [Pull request #1609: Update hex value for secondary text to improve contrast](https://github.com/alphagov/city-frontend/pull/1609).
- [Pull request #1620: Only add underline to back link when href exists](https://github.com/alphagov/city-frontend/pull/1620).
- [Pull request #1631: Fix classes on character count when in error state](https://github.com/alphagov/city-frontend/pull/1631).
...
- [Pull request #1576: Allow `lang` to be set on title and main of template](https://github.com/alphagov/city-frontend/pull/1576).

#### Add new override class to hide elements

You can now use the `.city-!-display-none` override class to hide elements.

- [Pull request #1586: Add display none override class](https://github.com/alphagov/city-frontend/pull/1586).

#### Visual updates to the warning text component

Align ‘Warning text’ icon with first line of the content fixing [#1352](https://github.com/alphagov/city-frontend/issues/1352) Some changes were made to the size and spacing of the icon to help with positioning.

- [Pull request #1578: Change position and spacing relationship of warning text icon](https://github.com/alphagov/city-frontend/pull/1578)

### Fixes

- [Pull request #1574: Make form elements scale correctly when text resized by user](https://github.com/alphagov/city-frontend/pull/1574).
- [Pull request #1584: Fix text resize issue with warning text icon](https://github.com/alphagov/city-frontend/pull/1584)
- [Pull request #1570: Prevent inputs ending up off screen or obscured by keyboards when linking from the error summary to inputs within a large fieldset](https://github.com/alphagov/city-frontend/pull/1570)
- [Pull request #1585: Explicitly set font weight on warning-text component](https://github.com/alphagov/city-frontend/pull/1585)
- [Pull request #1587: Fix height and alignment issue within header in Chrome 76+](https://github.com/alphagov/city-frontend/pull/1587)
- [Pull request #1589: Remove role="button" from header button](https://github.com/alphagov/city-frontend/pull/1589)
- [Pull request #1595: Do not output conditionally revealed content for radios or checkboxes when it's empty](https://github.com/alphagov/city-frontend/pull/1595)
...
- [Pull request #1553: Include formGroup on character count and pass through to textarea to allow class to be added to character count form group](https://github.com/alphagov/city-frontend/pull/1553). Thanks to [Emma Lewis](https://github.com/LBHELewis).

### Fixes

- [Pull request #1548: Fix fieldset legend text clipping when using a custom or fallback font](https://github.com/alphagov/city-frontend/pull/1548).
- [Pull request #1559: Stop IE8 from downloading GDS Transport font](https://github.com/alphagov/city-frontend/pull/1559).
- [Pull request #1564: Ensure legacy styles do not override button active state](https://github.com/alphagov/city-frontend/pull/1564).
...
[Pull request #1541: Allow date input fieldset attributes to be set](https://github.com/alphagov/city-frontend/pull/1541). Thanks to [andrew-mcgregor](https://github.com/andrew-mcgregor) for raising this.

#### Add ARIA role to the fieldset component

You can now pass an ARIA role to the fieldset component.

[Pull request #1541: Allow date input fieldset attributes to be set](https://github.com/alphagov/city-frontend/pull/1541).
...
- [Pull request #1527: Add inputmode option to the input component](https://github.com/alphagov/city-frontend/pull/1527)

### Fixes

- [Pull request #1523: Improve accessibility of details component by polyfilling only where the native element is not available](https://github.com/alphagov/city-frontend/pull/1523).
- [Pull request #1512: Update components to only output items when they are defined](https://github.com/alphagov/city-frontend/pull/1512).
- [Pull request #1538: Simplify button types to avoid unnecessary type attribute](https://github.com/alphagov/city-frontend/pull/1538).
- [Pull request #1542: Only default two thirds/one-third layout when footer has two sections](https://github.com/alphagov/city-frontend/pull/1542).
...
Add `city/` after `city-frontend/` to `@import` paths in your [Sass](https://sass-lang.com/) file.
...
If you’ve [added `node_modules/city-frontend` as a Sass include path](https://github.com/alphagov/city-frontend/blob/master/docs/installation/installing-with-npm.md#optional-resolving-scss-import-paths), add `city/` to your `@import` paths:
...
You must add `city/` to your import paths.

If you're importing `node_modules/city-frontend/all.js`, change this import path to `node_modules/city-frontend/city/all.js`.

If you’re importing a specific path, add `city/` after `city-frontend/`. For example, if you're importing the button component:
...
You do not need to do anything if you're using Nunjucks macros and the [`initAll`](https://github.com/alphagov/city-frontend/blob/master/docs/installation/installing-with-npm.md#option-1-include-javascript) function.

If you are not using Nunjucks macros, add a `city-` prefix to `data-module` attribute values. For example:
...
- `data-module="city-button"` to each `<button>` HTML tag
- `data-module="city-details"` to each `<details>` HTML tag

If you're using your own JavaScript code to initialise components, add a `city-` prefix to any selectors that find components using the `data-module` attribute.

[Pull request #1443: Ensure GOV.UK Frontend component selectors cannot conflict when initialised](https://github.com/alphagov/city-frontend/pull/1443)
...
- `js-character-count` to `city-js-character-count`
- `js-header-toggle` in the GOV.UK Frontend header component to `city-js-header-toggle`

[Pull request #1444: Renames `js-` css prefix to `city-js-`](https://github.com/alphagov/city-frontend/pull/1444)

##### If you’re using Nunjucks

1. Change the list of paths in `nunjucks.configure` so that the only GOV.UK Frontend path is `node_modules/city-frontend/`:
...
2. If you've extended the [page template](https://design-system.service.gov.uk/styles/page-template/), add `city/` to the template path:

```nunjucks
{% extends "city/template.njk" %}
```

3. Change the import paths in your components so they include `city/components/`. For example:
...
In the assets path, add `city/` after `city-frontend/`:
...
- [#1458: Namespace nunjucks and components](https://github.com/alphagov/city-frontend/pull/1458)
- [#1467: Update the main entry point in package.json](https://github.com/alphagov/city-frontend/pull/1467)
...
If you've extended or created components, you can no longer use the `city-focusable` or `city-focusable-fill` Sass mixins.

If you're using `city-focusable`, you must remove it. There’s no direct replacement, so you must [use our Sass variables to make your components consistent](https://design-system.service.gov.uk/get-started/focus-states/#make-other-focusable-elements-accessible) with GOV.UK Frontend.

If you're using `city-focusable-fill`, include the `city-focused-text` mixin inside your component's `:focus` selector. For example:
...
- [#1309: Update links (and things that look like links) to use the new focus style](https://github.com/alphagov/city-frontend/pull/1309)
- [#1312: Update form inputs focus to comply with WCAG 2.1](https://github.com/alphagov/city-frontend/pull/1312)
- [#1313: Add new focus style to buttons](https://github.com/alphagov/city-frontend/pull/1315)
- [#1321: Update footer links to use new focus style](https://github.com/alphagov/city-frontend/pull/1321)
- [#1324: Update accordion to use new WCAG 2.1 compliant focus style](https://github.com/alphagov/city-frontend/pull/1324)
- [#1326: Update tabs component to WCAG 2.1 compliant focus style](https://github.com/alphagov/city-frontend/pull/1326)
- [#1361: Remove `city-focusable`, `city-focusable-fill` mixins, introduce `city-focus-text` mixin](https://github.com/alphagov/city-frontend/pull/1361)
...
If you're using legacy projects like GOV.UK Elements, you can keep your current colours by [turning on compatibility mode](https://github.com/alphagov/city-frontend/blob/master/docs/installation/compatibility.md).
...
[Pull request #1288: Update colour palette](https://github.com/alphagov/city-frontend/pull/1288).
...
If you're using GOV.UK Frontend and [GOV.UK Template](https://github.com/alphagov/city_template), you can [turn on compatibility mode](https://github.com/alphagov/city-frontend/blob/master/docs/installation/compatibility.md) to keep using the font from GOV.UK Template.

Pull requests:

- [#1434: Update font to use v2 of GOV.UK Transport font](https://github.com/alphagov/city-frontend/pull/1434)
- [#1441: Removing font style adjustments](https://github.com/alphagov/city-frontend/pull/1441)
...
[Pull request #1426: Make radios and checkboxes components easier to link to from error summary](https://github.com/alphagov/city-frontend/pull/1426)

#### Update the markup for tabs

You do not need to do anything if you're using Nunjucks macros.

If you are not using Nunjucks macros, remove the `city-tabs__tab--selected` class from the first tab's link, then add the `city-tabs__list-item--selected` class to that link's parent list item.
...
[Pull request #1496: Update the focus state for tabs](https://github.com/alphagov/city-frontend/pull/1443)
...
- remove the `.city-button--start` class
...
[Pull request #1341: Add new start button icon](https://github.com/alphagov/city-frontend/pull/1341)
...
[Pull request #1345: Set 'vertical-align:top' positioning on table headers and cells](https://github.com/alphagov/city-frontend/pull/1345)

#### Replace Sass mixins in grids

If you're using the `@city-grid-column` Sass mixin to create custom grid classes, you must remove the `$class` parameter.
...
You can no longer use the `city-grid-row` mixin. You can replace it with the `.city-grid-row` class in your HTML.

You must also replace calls to the `grid-width` mixin with calls to the `city-grid-width` mixin.

Pull requests:

- [#1376: Remove $class param from @city-grid-column mixin](https://github.com/alphagov/city-frontend/pull/1376)
- [#1342: Remove grid-width mixin](https://github.com/alphagov/city-frontend/pull/1342)
- [#1343: Removes city-grid-row mixin](https://github.com/alphagov/city-frontend/pull/1343)

#### Rename the border width variable

If you use Sass and you’ve extended or created components that use the border width variable, rename `$city-border-width-mobile` to `$city-border-width-narrow`.

[Pull request #1287: Rename border-width-mobile to reflect how it's used](https://github.com/alphagov/city-frontend/pull/1287)
...
[Pull request #1367: Allow for classes, rowspan, colspan and attributes on row headers](https://github.com/alphagov/city-frontend/pull/1367). Thanks to [edwardhorsford](https://github.com/edwardhorsford).

#### Use page wrapper auto spacing

You can now add the `.city-main-wrapper--auto-spacing` modifier class to your `<main>` element to add responsive padding to the top and bottom of the page.

This will add the correct amount of padding depending on if there are elements above the `<main>` element inside the `city-width-container` wrapper. Elements above the `<main>` element could include a back link or breadcrumb component.

If you need to control the spacing manually, use the `.city-main-wrapper--l` modifier instead.

The `city-main-wrapper` and `city-main-wrapper--l` Sass mixins are now deprecated. [Contact us](https://design-system.service.gov.uk/get-in-touch/) if you need to continue using these mixins.

[Pull request #1493: Add automatic vertical spacing modifier for main wrapper](https://github.com/alphagov/city-frontend/pull/1493)
...
[Pull request #1434: Update font to use v2 of GOV.UK Transport font](https://github.com/alphagov/city-frontend/pull/1434). Thanks to [@Nooshu](https://github.com/Nooshu).

### Fixes

- [Pull request #1310: The border on error summaries is now the correct width on mobile.
  GOV.UK Template's focused link colour no longer overrides GOV.UK Frontend](https://github.com/alphagov/city-frontend/pull/1310).
- [Pull request #1316: Checkboxes and radios no longer have a transparent outline - because it's no longer needed to fix custom focus state colours](https://github.com/alphagov/city-frontend/pull/1316).
- [Pull request #1324: There’s no longer an outline when a user focuses an accordion element in Firefox](https://github.com/alphagov/city-frontend/pull/1324).
- [Pull request #1330: We've changed the spacing around lists in tabs on mobile and with JavasScript disabled, so they're consistent with other lists](https://github.com/alphagov/city-frontend/pull/1330).
- [Pull request #1351: You can now use HTML elements in the labels for tabs](https://github.com/alphagov/city-frontend/pull/1351).
- [Pull request #1353: Example HTML code in the Design System now has correct indenting](https://github.com/alphagov/city-frontend/pull/1353).
- [Pull request #1368: We’ve clarified how you should use `productName` and `serviceName` in the header component](https://github.com/alphagov/city-frontend/pull/1368) - thanks to [@edwardhorsford](https://github.com/edwardhorsford).
- [Pull request #1359: Users can now use the Tab key to highlight tabs in IE8](https://github.com/alphagov/city-frontend/pull/1359).
- [Pull request #1370: Each submit button on a page now has its own double-click timer, so clicking one button will not disable the other buttons](https://github.com/alphagov/city-frontend/pull/1370).
- [Pull request #1381: You can now import the Sass settings files without needing to import the other settings first](https://github.com/alphagov/city-frontend/pull/1381).
- [Pull request #1442: Character count components are now hidden to assistive technologies when they're not visible](https://github.com/alphagov/city-frontend/pull/1442).
- [Pull request #1434: The underline below links no longer sits too far down in Firefox](https://github.com/alphagov/city-frontend/pull/1434).
- [Pull request #1435: When a user resubmits a form, the error summary is now correctly focused instead of the form](https://github.com/alphagov/city-frontend/pull/1435).
- [Pull request #1473: We’ve removed icon-arrow-left.png and icon-important.png, because they were not used in GOV.UK Frontend](https://github.com/alphagov/city-frontend/pull/1473)
- [Pull request #1497: Users can now conditionally reveal content on pages with multiple grouped radios](https://github.com/alphagov/city-frontend/pull/1497) - thanks to [@colinrotherham](https://github.com/colinrotherham) and [@frankieroberto](https://github.com/frankieroberto) for their help.
...
- You can now override the visually hidden 'Support links' text in the footer by
  ...
  ([Pull request #1387](https://github.com/alphagov/city-frontend/pull/1387))
...
- Secondary buttons and warning buttons now have the correct background colour
  ...
  ([Pull request #1392](https://github.com/alphagov/city-frontend/pull/1392))
...
- The crown logo image in the header now:
  ...
  ([Pull request #1419](https://github.com/alphagov/city-frontend/pull/1419))
...
- Support custom attributes on summary list action links
  ...
  ([PR #1372](https://github.com/alphagov/city-frontend/pull/1372))
...
- Support aria-describedby on all form fields
  ...
  ([PR #1347](https://github.com/alphagov/city-frontend/pull/1347))
...
- Update colour for MHCLG
  ...
  ([PR #1319](https://github.com/alphagov/city-frontend/pull/1319))

- Remove deprecated `@else-if` statement, replace with `@else if`

  ([PR #1333](https://github.com/alphagov/city-frontend/pull/1333))
...
- Prevent the fallback PNG image for the crown in the header from being
  ...
  ([PR #1337](https://github.com/alphagov/city-frontend/pull/1337))
...
- Add new secondary and warning button variants
  ...
  ([PR #1207](https://github.com/alphagov/city-frontend/pull/1207))

- Add new city-shade and city-tint functions for creating shades and tints of
  colours.
...
- Add support for custom row classes on the summary list component (including support for some rows without action links)
  ...
  ([PR #1259](https://github.com/alphagov/city-frontend/pull/1259))
...
- Ensure fieldset never exceeds max-width
  ...
  See discussion in ["Reset your fieldset"](https://thatemil.com/blog/2015/01/03/reset-your-fieldset/) and raised by [issue #1264](https://github.com/alphagov/city-frontend/issues/1264)

  ([PR #1269](https://github.com/alphagov/city-frontend/pull/1269))
...
- Add smaller versions of radio buttons and checkboxes
  ...
  ([PR #1125](https://github.com/alphagov/city-frontend/pull/1125))

🔧 Fixes:

- Prevent duplicate checkbox aria-describedby

  Addresses an edge case where a checkbox with a hint (but without a fieldset) is output with duplicate `aria-describeby` attributes. Fixes issue [#1248](https://github.com/alphagov/city-frontend/pull/1248).

  Thanks to [@colinrotherham](https://github.com/colinrotherham) for fixing this issue.

  ([PR #1265](https://github.com/alphagov/city-frontend/pull/1265))
...
- Add classes to action items in the summary list component
  ...
  ([PR #1233](https://github.com/alphagov/city-frontend/pull/1233))

- Allow initAll to be scoped to a specific part of a page

  See ["Initialise GOV.UK Frontend in only certain sections of a page"](docs/installation/installing-with-npm.md#initialise-city-frontend-in-only-certain-sections-of-a-page) for more information.

  ([PR #1216](https://github.com/alphagov/city-frontend/pull/1216))
...
- Fix tabs bullet point character encoding issue when not enhanced
  ...
  ([PR #1247](https://github.com/alphagov/city-frontend/pull/1247))

- Update padding of city-main-wrapper

  This increases the padding of `city-main-wrapper` (on tablet and above) to be more inline with GOV.UK. When updating, your pages will have 10px more white space above and below the 'main' content area.

  ([PR #1073](https://github.com/alphagov/city-frontend/pull/1073))

- Remove error-summary dependence on document.onload

  ([PR #1215](https://github.com/alphagov/city-frontend/pull/1215))
...
- Ensure font is not loaded twice on slow networks
  ...
  ([PR #1242](https://github.com/alphagov/city-frontend/pull/1242))

## 2.8.0 (Feature release)

### Considerations for fixed components such as modals

We have been made aware that the [change to force scroll bars to appear at all times](https://github.com/alphagov/city-frontend/pull/1230) can change the behaviour of some components such as modals.
...
- Enable `pattern` attribute for input
  ...
  ([PR #1172](https://github.com/alphagov/city-frontend/pull/1172))
...
- Prevent horizontal jump as scrollbars appear
  ...
  <https://github.com/alphagov/city-frontend/issues/1204>

  ([PR #1230](https://github.com/alphagov/city-frontend/pull/1230))
...
- Accommodate camera notches on new devices (iPhone X, Google Pixel 3 etc)
  ...
  ([PR #1176](https://github.com/alphagov/city-frontend/pull/1176))

- Prefix error messages with a visually hidden "Error:", to make it clearer to
  users of assistive technologies

  ([PR #1221](https://github.com/alphagov/city-frontend/pull/1221))
...
- Prevent accidental multiple submissions of forms
  ...
  ```html
  <button class="city-button" data-prevent-double-click="true">
    Submit
  </button>
  ```
  ...
  ([PR #1018](https://github.com/alphagov/city-frontend/pull/1018))
...
- Ensure that files within the core, objects and overrides layers can be
  ...
  their dependencies (for example, most of them require the city-exports mixin
  ...
  ([PR #1235](https://github.com/alphagov/city-frontend/pull/1235))
...
- Ensure inset component does not misalign nested components
  ...
  ([PR #1232](https://github.com/alphagov/city-frontend/pull/1232))
...
- Improve word wrapping in summary list component
  ...
  Also thanks to [Malcolm Butler](https://github.com/MoJ-Longbeard) for exploring a [previous version of this fix](https://github.com/alphagov/city-frontend/pull/1185).

  ([PR #1220](https://github.com/alphagov/city-frontend/pull/1220))
...
- Added config file for prototype kit.
  ...
  ([PR #1102](https://github.com/alphagov/city-frontend/pull/1102))
...
- Disable incorrect iOS/Edge text size adjustments
  ...
  ([PR #1178](https://github.com/alphagov/city-frontend/pull/1178))
...
- Fix wrapping of long lines of text in summary list
  ...
  ([PR #1169](https://github.com/alphagov/city-frontend/pull/1169))
...
- Enable `autocomplete` attributes for input components.
  ...
  ([PR #1146](https://github.com/alphagov/city-frontend/pull/1146))
...
- Include Accordion component in global namespace
  ...
  ([PR #1157](https://github.com/alphagov/city-frontend/pull/1157))
...
- Fix Checkboxes and Radios targeting selectors outside it's scope
  ...
  ([PR #1156](https://github.com/alphagov/city-frontend/pull/1156))

- Fixes styling of the accordion component when there is no JavaScript or it has been turned off

  Thanks @dankmitchell for reporting this issue ([#1130](https://github.com/alphagov/city-frontend/issues/1130))

  ([PR #1149](https://github.com/alphagov/city-frontend/pull/1149))

- Remove hover state for accordion sections on mobile

  ([PR #1148](https://github.com/alphagov/city-frontend/pull/1148))
...
- Fix container not being centered in IE8
  ...
  ([PR #1147](https://github.com/alphagov/city-frontend/pull/1147))
...
- Make gutters in the header consistent with the grid
  ...
  ([PR #1144](https://github.com/alphagov/city-frontend/pull/1144))
...
- Update summary list to simplify actions
  ...
  ([PR #1131](https://github.com/alphagov/city-frontend/pull/1131))
...
- Accordion component 🎉
  ...
  ([PR #958](https://github.com/alphagov/city-frontend/pull/958))
...
- Add desktop specific grid column widths
  ...
  ([PR #1094](https://github.com/alphagov/city-frontend/pull/1094))
...
- Add summary list component
  ...
  ([PR #1065](https://github.com/alphagov/city-frontend/pull/1065))

🔧 Fixes:

- Update visually hidden class to fix ordering issue in VoiceOver OSX

  We have fixed an issue in VoiceOver OSX where using the `city-visually-hidden` class in links would result in the text being announced out of order.
  ...
  ([PR #1109](https://github.com/alphagov/city-frontend/pull/1109))
...
- Improve rendering in older Internet Explorer
  ...
  ([PR #1119](https://github.com/alphagov/city-frontend/pull/1119))
...
- Remove unused hint, error message and label imports from the Character Count
  ...
  ([PR #1087](https://github.com/alphagov/city-frontend/pull/1087))

- Warning text component, remove negative margin left and reduce padding left to match.

  ([PR #1084](https://github.com/alphagov/city-frontend/pull/1084))
...
- Add 5px bottom margin to list items within lists that do not have bullets or
  ...
  ([PR #1078](https://github.com/alphagov/city-frontend/pull/1078))

- Add error message attributes to all components

  ([PR #1086](https://github.com/alphagov/city-frontend/pull/1086))
...
- Scroll to label or legend when linked from error summary
  ...
  ([PR #1056](https://github.com/alphagov/city-frontend/pull/1056))
...
- Allow form group classes on date, file upload, input, select and textarea
  ...
  ([PR #1059](https://github.com/alphagov/city-frontend/pull/1059))

🔧 Fixes:

- Remove implicit dependency on Element for classList

  ([PR #1063](https://github.com/alphagov/city-frontend/pull/1063))
...
- Single field with error should have 'aria-describeby' attribute
  ...
  ([PR #1054](https://github.com/alphagov/city-frontend/pull/1054))
...
- Add important parameter to visually hidden sass mixins
  ...
  Optional parameter added to `city-visually-hidden` and `city-visually-hidden-focusable` mixins (defaults to true)

  ([PR #1037](https://github.com/alphagov/city-frontend/pull/1037))
...
- Add support for attributes on table cells
  ...
  ([PR #1045](https://github.com/alphagov/city-frontend/pull/1045))
...
- Allow form group classes on radios and checkboxes
  ...
  ([PR #1043](https://github.com/alphagov/city-frontend/pull/1043))

🔧 Fixes:

- Fix anchor links in tabs component panels

  ([PR #1031](https://github.com/alphagov/city-frontend/pull/1031))

- Fix IE8 support in builds in the `dist/` folder

  ([PR #1035](https://github.com/alphagov/city-frontend/pull/1035))
...
- Allow classes on table header and row cells
  ...
  ([PR #1015](https://github.com/alphagov/city-frontend/pull/1015))

- Add character count component

  ([PR #959](https://github.com/alphagov/city-frontend/pull/959))
...
- Apply max-width to the `<select>` element
  ...
  ([PR #1013](https://github.com/alphagov/city-frontend/pull/1013))
...
- Prevent product name in header from wrapping
  ...
  ([PR #1007](https://github.com/alphagov/city-frontend/pull/1007))

- Set text colour for radios divider

  ([PR 1023](https://github.com/alphagov/city-frontend/pull/1023))
...
- Stop links styled as button from being dragged
  ...
  ([PR #1020](https://github.com/alphagov/city-frontend/pull/1020))
...
- Allow additional 'meta' content in the footer
  ...
  ([PR #990](https://github.com/alphagov/city-frontend/pull/990))
...
- Allow attributes to be added to some child items in header, footer, breadcrumbs, tabs and error-summary components
  ...
  ([PR #993](https://github.com/alphagov/city-frontend/pull/993))
...
- Fix issue with conditional form content and inline form controls
  ...
  ([PR #970](https://github.com/alphagov/city-frontend/pull/970))
...
- Add component options (arguments) as `macro-options.json` to `package`
  ...
  ([PR #998](https://github.com/alphagov/city-frontend/pull/998))
...
- Update `browsersList` in `package.json` to reflect our supported browsers
  ...
  ([PR #1002](https://github.com/alphagov/city-frontend/pull/1002))
...
- Set panel component's title heading level to 1 as default
  ...
  ([PR #967](https://github.com/alphagov/city-frontend/pull/967))

- Remove deprecated `city-visually-hidden-focussable` class name

  In 1.1 release we added a new, correctly spelt
  `city-visually-hidden-focusable` CSS class and marked the old one as
  deprecated.

  To migrate you need to change `city-visually-hidden-focussable` to
  `city-visually-hidden-focusable` in your codebase.

  ([PR #968](https://github.com/alphagov/city-frontend/pull/968))

- Remove name-based width logic from date-input component

  In [1.1 release](https://github.com/alphagov/city-frontend/pull/857/files#diff-e94394b2ac1d4f73991af98e4fa34fa3L32)
  ...
  as `city-input--width-2` or `city-input--width-4` to the input fields.

  ([PR #969](https://github.com/alphagov/city-frontend/pull/969))
...
- Rename `name` argument of date-input component to `namePrefix`.
  ...
  ([PR #984](https://github.com/alphagov/city-frontend/pull/984))

- Turn off [compatibility mode](./docs/installation/installing-with-npm.md#compatibility-mode) by default for [GOV.UK Elements](https://github.com/alphagov/city_elements), [GOV.UK Template](https://github.com/alphagov/city_template), [GOV.UK Frontend Toolkit](https://github.com/alphagov/city_frontend_toolkit)
  ...
  ```SCSS
  // application.scss
  $city-compatibility-cityfrontendtoolkit: true;
  $city-compatibility-citytemplate: true;
  $city-compatibility-cityelements: true;
  @import "city-frontend/all";
  ```

  ([PR #981](https://github.com/alphagov/city-frontend/pull/981))
...
- Turn on relative typography (rem) by default
  ...
  If you need to change this setting for compatibility with [GOV.UK Elements](https://github.com/alphagov/city_elements), [GOV.UK Template](https://github.com/alphagov/city_template), [GOV.UK Frontend Toolkit](https://github.com/alphagov/city_frontend_toolkit) consider enabling [compatibility mode](./docs/installation/installing-with-npm.md#compatibility-mode).

  Otherwise, set `$city-typography-use-rem` to `false` before importing GOV.UK Frontend styles into your app:

  ```SCSS
  // application.scss
  $city-typography-use-rem: false;
  @import "city-frontend/all";
  ```
...
- Remove anchor styling in city-lists
  ...
  To migrate we recommend using adding `.city-link` class to any anchors within a city-list.

  If you would like to replicate this functionality without adding a class you can also enable [global styles](https://github.com/alphagov/city-frontend/blob/master/docs/installation/installing-with-npm.md#global-styles) (Note: global styles are enabled by default in the GOV.UK Prototype Kit)

  ([PR #985](https://github.com/alphagov/city-frontend/pull/985))

- Remove the width declaration from the `<select>` component

  The `<select>` component’s width will now be defined by it’s content. This addresses some accessibility issues with the select being 100% wide by default. If you want to style your select to be 100% wide we have added a new override class `.city-!-width-full` to allow this.

  ([PR #960](https://github.com/alphagov/city-frontend/pull/960))
...
- Use text colour on focus for better contrast
  ...
  ([PR #982](https://github.com/alphagov/city-frontend/pull/982))

🆕 New features:

- Add a new width override class `.city-!-width-full`
...
- Allow attributes on select items
  ...
  ([PR #977](https://github.com/alphagov/city-frontend/pull/977))
...
- Textareas can now only be resized vertically, to prevent them being resized
  ...
  ([PR #976](https://github.com/alphagov/city-frontend/pull/976))

- Defend tables against GOV.UK Elements code
  ([PR #983](https://github.com/alphagov/city-frontend/pull/983))
...
- Allow attributes on checkboxes/radios
  ...
  ([PR #942](https://github.com/alphagov/city-frontend/pull/942))
...
- Fix incorrect panel title bottom margin with optional text
  ...
  ([PR #936](https://github.com/alphagov/city-frontend/pull/936))
...
- Remove template whitespace
  ...
  ([PR #949](https://github.com/alphagov/city-frontend/pull/949))
...
- Remove additional dotted outline from focussed buttons in Firefox
  ...
  ([PR #951](https://github.com/alphagov/city-frontend/pull/951))

- Update date input component to use `display: inline-block`
  ([PR #938](https://github.com/alphagov/city-frontend/pull/938))

- Change spacing relationship on default and small legends and hints
  ([PR #940](https://github.com/alphagov/city-frontend/pull/940))

- Adjust tag component padding to compensate for font spacing
  ([PR #955](https://github.com/alphagov/city-frontend/pull/955))
...
- Allow for optional divider between radio items
  ...
  ([PR #849](https://github.com/alphagov/city-frontend/pull/849))
...
- Allow og:image meta tag url to be set independantly
  ...
  ([PR #847](https://github.com/alphagov/city-frontend/pull/847))
...
- Only underline the logo in the header on underline when users have overridden
  ...
  ([PR #926](https://github.com/alphagov/city-frontend/pull/926))
...
- Allow for optional hint for each radio and checkbox item
  ...
  ([PR #846](https://github.com/alphagov/city-frontend/pull/846))
...
- Allow additional classes to be added to the radio and checkbox items
  ...
  ([PR #880](https://github.com/alphagov/city-frontend/pull/880))
...
- Fix flash of unstyled content with conditional reveals (Radios and Checkboxes)
  ...
  ([PR #885](https://github.com/alphagov/city-frontend/pull/885))
...
- Update details behaviour to remove margin-bottom for all elements
  ([PR #900](https://github.com/alphagov/city-frontend/pull/900))

- Update internal padding of tab content in the tabs component
  ([PR #886](https://github.com/alphagov/city-frontend/pull/886))
...
- Fixes an issue where clicking the revealed content in browsers that do not
  ...
  ([PR #912](https://github.com/alphagov/city-frontend/pull/912))
...
- Fixes an issue where it's not possible to make any field that does not have
  ...
  ([PR #908](https://github.com/alphagov/city-frontend/pull/908))

- Fix undefined class displaying in date input
  ([PR #913](https://github.com/alphagov/city-frontend/pull/913))

## 1.1.0 (Feature release)

🆕 New features:

- Add `city-visually-hidden-focusable` class

  Adds `.city-visually-hidden-focusable` and deprecates `.city-visually-hidden-focussable` in order to fix the typo in the class name. Please consider updating your code as the next major release will remove the deprecated class.

  ([PR #859](https://github.com/alphagov/city-frontend/pull/859))
...
- Allow panel component title heading to be customisable.
  ...
  ([PR #853](https://github.com/alphagov/city-frontend/pull/853))
...
- Update date input component
  ...
  ([PR #857](https://github.com/alphagov/city-frontend/pull/857))
...
- The typography scale can now be converted from pixels to rem automatically,
  ...
  `$city-typography-use-rem: true` and ensure that `$city-root-font-size` is
  ...
  that use alphagov/city_template this should be 10px.

  The intention is to enable this by default in the next major version:
  <https://github.com/alphagov/city-frontend/issues/868>

  ([PR #858](https://github.com/alphagov/city-frontend/pull/858))

🔧 Fixes:

- Remove mistakenly applied 5px bottom margin from radio and checkbox
  labels.
  ([PR #883](https://github.com/alphagov/city-frontend/pull/883))

- Apply `display:block` to `.city-main-wrapper`

  In IE11 `main` element is set to `display:inline` so padding
  and margins aren't applied.
  ([PR #863](https://github.com/alphagov/city-frontend/pull/863)))
...
- Line-heights are now converted from pixels to relative 'unit-less' values
  ...
  ([PR #837](https://github.com/alphagov/city-frontend/pull/837) and
  [PR #848](https://github.com/alphagov/city-frontend/pull/848))
...
- Add bottom margin to Tabs component
  ...
  ([PR #841](https://github.com/alphagov/city-frontend/pull/841))
...
- Update Crown copyright link
  ...
  ([PR #824](https://github.com/alphagov/city-frontend/pull/824))

- Fixes radio and checkbox labels extending full width of page
  ([PR #821](https://github.com/alphagov/city-frontend/pull/821))
...
- Prevent the exclamation mark in the warning text component from being
  ...
  of text
  ([PR #856](https://github.com/alphagov/city-frontend/pull/856))
...
- Add customised colours handling for warning text
  ...
  ([PR #852](https://github.com/alphagov/city-frontend/pull/852))

- Fixes a bug where the phase banner incorrectly uses a font-size of 19px when
  global styles are enabled
  ([PR #877](https://github.com/alphagov/city-frontend/pull/877))
...
- Add outlines to Radios and Checkboxes for customised colour users
  ...
  ([PR #854](https://github.com/alphagov/city-frontend/pull/854))

- Add outline to tag for customised colour users

  Now when a [user customises their colours](https://accessibility.blog.gov.uk/2017/03/27/how-users-change-colours-on-websites/),
  the tag component still keeps it's meaning.
  ([PR #855](https://github.com/alphagov/city-frontend/pull/855))
...
- Define size of table in `px` rather than `em`
  ...
  ([PR #845](https://github.com/alphagov/city-frontend/pull/845))
...
- Fix header component's reliance on markup whitespace
  ...
  ([PR #884](httqps://github.com/alphagov/city-frontend/pull/884))

🏠 Internal:

- Fix Design System url in package READMEs and review app
  ([PR #812](https://github.com/alphagov/city-frontend/pull/812))

- Update back-link example to show default usage doesn't need
  `text` parameter
  ([PR #819](https://github.com/alphagov/city-frontend/pull/819))

- Lowercase component names
  ([PR #822](https://github.com/alphagov/city-frontend/pull/822))

- Tidy up some package.json fields to help contributors and users
  ([PR #827](https://github.com/alphagov/city-frontend/pull/827))

- Fix failing Heroku app with Node 10.5.0
  Revert to pinning node version in package json
  ([PR #833](https://github.com/alphagov/city-frontend/pull/833))

- Format YAML and Nunjucks consistently
  ([PR #830](https://github.com/alphagov/city-frontend/pull/830))
...
- Add default text for back-link component
  ([PR #793](https://github.com/alphagov/city-frontend/pull/793))

- Add default container class to the header component
  ([PR #807](https://github.com/alphagov/city-frontend/pull/807))
...
- Add tabs component – thanks to [@adamsilver](https://github.com/adamsilver) and [@trevorsaint](https://github.com/trevorsaint) for contributing
  ([PR #776](https://github.com/alphagov/city-frontend/pull/776))
...
- Reduce margin-bottom on the hint when following a default or small labe
  ...
  `<label>` or `<label class="city-label--s">`.
  ([PR #806](https://github.com/alphagov/city-frontend/pull/806))

🏠 Internal:

- Remove instructions to login with npm, which is no longer required
  ([PR #795](https://github.com/alphagov/city-frontend/pull/795))

- Update docs with the assistive technology we support ([PR #800](https://github.com/alphagov/city-frontend/pull/800))

- Update docs about installing fonts ([PR #802](https://github.com/alphagov/city-frontend/pull/802))

- Update browser support matrix
  Remove Windows Phone
  Update IE 8-10 to functional and IE 11 to compliant
  ([PR #803](https://github.com/alphagov/city-frontend/pull/803)

- Add notice about the use of `html` arguments in Nunjucks macros for production
  ([PR #785](https://github.com/alphagov/city-frontend/pull/785))

- Add CHANGELOG_TEMPLATE to make changelogs easier for new contributors
  ([PR #798](https://github.com/alphagov/city-frontend/pull/798))

- Update package `README`
  ([PR #804](https://github.com/alphagov/city-frontend/pull/804))

- Update JavaScript installation instructions
  ([PR #808](https://github.com/alphagov/city-frontend/pull/808))

## 0.0.32 (Breaking release)

**This release changes the name of package.** It's now published as `city-frontend` on `npm`.
...
- The default build of the GOV.UK Frontend JavaScript now does not initialize all components automatically.
  ...
  ```html
    <script src="{path-to-city-frontend}/all.js"></script>
    <script>window.cityFrontend.initAll()</script>
  ```
...
- Consistently structure the Details and Button component, so that they can be instantiated the same as the other components.
  ...
  If you're using `cityFrontend.initAll()` you do not need to make any changes, otherwise you need to change
  ...
  ([PR #761](https://github.com/alphagov/city-frontend/pull/761))
...
- All sass-mq settings have now been made private. We are now exposing new
  ...
  - `$city-breakpoints` - Map of breakpoint definitions
  - `$city-show-breakpoints` - Whether to show the current breakpoint in the
    top right corner
  - `$city-ie8-breakpoint` - Breakpoint to rasterize to for IE8

  If you are overriding any settings prefixed with `$mq-` in your application
  you will need to update to the use the new `$city-` prefixed settings.

  ([PR #748](https://github.com/alphagov/city-frontend/pull/748))

- Font settings have been renamed:

  - `$city-font-stack` has been renamed to `$city-font-family`
  - `$city-font-stack-tabular` has been renamed to `$city-font-family-tabular`
  - `$city-font-stack-print` has been renamed to `$city-font-family-print`
...
- Spacing has been refactored. You will need to update Sass that currently uses GOV.UK Frontend spacing:
  ...
  where `*` is the number on the spacing scale. The scale itself has remained the same so that `$city-spacing-scale-3` corresponds to `city-spacing(3)`. This change allows us to control the error messaging when incorrect values are used and to deprecate variables. The values of spacing variables can also be overridden by consumers.

  - Instead of:

  ```css
  @include city-responsive-margin($city-spacing-responsive-2, "bottom");
  @include city-responsive-padding($city-spacing-responsive-2, "bottom");
  ```

  use

  ```css
  @include city-responsive-margin(2, "bottom");
  @include city-responsive-padding(2, "bottom");
  ```

  This change, again, allows us to control the error messaging since spacing variables are not exposed directly. Also, the spacing scale itself has not changed so that `$city-spacing-responsive-2` corresponds to `2` when passed to the padding and margin mixins.

  This PR also updates tests and sass-docs of spacing variables and helpers.

  Additionally, this PR hardcodes the value of `$city-gutter`, see PR for more details.

  ([PR #779](https://github.com/alphagov/city-frontend/pull/779))

- Remove `pageStart` block from template, as could result in rendering issues in older IE.
  ([PR #765](https://github.com/alphagov/city-frontend/pull/765))

- You should no longer call the `mq` mixin directly - you should replace any
  calls to it from your own code with `city-media-query` which accepts the same
  ...
  ([PR #763](https://github.com/alphagov/city-frontend/pull/763))

- All of the shorthand 'font' mixins (e.g. `city-font-bold-80`,
  `city-font-regular-tabular-19`), have been removed and replaced with calls to
  a new mixin `city-font`. If you are using these mixins in your application
  you will need to update your code to call `city-font` instead.

  ([PR #772](https://github.com/alphagov/city-frontend/pull/772))

- The font maps are no longer as individual variables (e.g. `$city-font-80`) -
  they are all now part of one single `$city-typography-scale` map. Instead of
  passing font maps to `city-typography-responsive` you should now pass the
  desktop font size (e.g. `city-typography-responsive(80)` or
  `city-typography-responsive($size: 80)`.

  ([PR #772](https://github.com/alphagov/city-frontend/pull/772))

- All organisation variables (e.g. `$city-cabinet-office`) have been moved into
  a single `$city-colours-organisations` map. If you need to use an
  organisation colour in your own code, you should use the new
  `city-organisation-colour` function:

  ```scss
  .element {
    color: city-organisation-colour(cabinet-office);
  }
  ```
...
- The colour palette variables (e.g. `$city-green`) have been moved into a
  new single `$city-colours` map. If you need to reference a colour within your
  application you should use the new `city-colour` function:

  ```scss
  .element {
    color: city-colour("green");
  }
  ```

  Tints (`$city-green-50`, `$city-green-25`) have been removed from the colour
  palette.
...
- The 'circle shape' object (`.city-circle`) which was used by the warning text
  component's '!' icon has been removed and the `city-warning-text__icon` class
  has been updated to make it circular without the need for another class.

  ([PR #782](https://github.com/alphagov/city-frontend/pull/782))

- Removal of `city-prose-scope`
  ...
  In version [0.0.29-alpha](https://github.com/alphagov/city-frontend/releases/tag/v0.0.29-alpha)
  ...
  ([PR #778](https://github.com/alphagov/city-frontend/pull/778))
...
- Make override classes consistently verbose
  ...
  - `.city-!-f-{size}` with `.city-!-font-size-{size}`
  - `.city-!-w-{weight}` with `.city-!-font-weight-{weight}`
  - `.city-!-m{direction}-r{scale}` with `.city-!-margin-{direction}-{scale}`
  - `.city-!-p{direction}-r{scale}` with `.city-!-padding-{direction}-{scale}`

  For example if you were using:

  - `.city-!-mb-r5` you would need to change this to `.city-!-margin-bottom-5`
  - `.city-!-f-24` you would need to change this to `.city-!-font-size-24`

  See the original Pull Request for the full list of classes before and after.

  ([PR #786](https://github.com/alphagov/city-frontend/pull/786))

🔧 Fixes:

- Fix city-equilateral-height function usage in shape-arrow helper
  ([PR ##766](https://github.com/alphagov/city-frontend/pull/766))
...
- The `<label>` element will now be omitted for form controls where no label
  ...
  ([PR #740](https://github.com/alphagov/city-frontend/pull/740))

- Add `city-main-wrapper` to `<main>` element by default.
  ([PR #742](https://github.com/alphagov/city-frontend/pull/742))
...
- Use relative imports whenever a component imports another component, to allow
  ...
  nunjucks paths.
  ([PR #743](https://github.com/alphagov/city-frontend/pull/743))

- Update JavaScript global namespace from 'all' to 'cityFrontend',
  we intend to allow users to initialize components from this namespace.
  ([PR #747](https://github.com/alphagov/city-frontend/pull/747))

- Tabular numbers will now correctly use the `$city-font-family-tabular`
...
- Prevents focus from being lost to the inline SVGs in the header (the crown)
  ...
  ([PR #774](https://github.com/alphagov/city-frontend/pull/774))

- Use the correct class name in bold label example
  (city-label--s rather than city-label--bold)
  ([PR #784](https://github.com/alphagov/city-frontend/pull/784))

- Update table of arguments for each component to ensure they're accurate.
  ([PR #769](https://github.com/alphagov/city-frontend/pull/769))

- Add explicit dependency on colour maps
  ([PR #790](https://github.com/alphagov/city-frontend/pull/790))

🆕 New features:

- Components are now available to use from the `cityFrontend` global.
...
- Most of the settings, helpers and tools layers is now documented using
  ...
  public.
  ([PR #748](https://github.com/alphagov/city-frontend/pull/748))
  ([PR #762](https://github.com/alphagov/city-frontend/pull/762))
...
- Improve release steps
  ([PR #745](https://github.com/alphagov/city-frontend/pull/745))

- Reintroduce mistakenly deleted HTML5Shiv required for IE8
  ([PR #749](https://github.com/alphagov/city-frontend/pull/749))

- Fix issues with canvas colour bleeding into main review page
  ([PR #741](https://github.com/alphagov/city-frontend/pull/741))

- Add header with service name and navigation variant to header README ([PR #781](https://github.com/alphagov/city-frontend/pull/781))

## 0.0.31-alpha (Breaking release)

💥 Breaking changes:

- The `city-file-url` helper has been renamed to to `city-image-url`. If you
  are using this function in your own styles you will need to update calls to
  it. ([PR #726](https://github.com/alphagov/city-frontend/pull/726))

- The `$city-global-images` variable has been renamed to to
  `$city-images-path`. If you are overriding this variable or using it in your
  own styles you will need to update your code to use the new variable name.
  ([PR #726](https://github.com/alphagov/city-frontend/pull/726))

🔧 Fixes:

- Namespacing SCSS exports with 'city' prefix to avoid clashes with
  exports in applications consuming GOV.UK Frontend
  ([PR #732](https://github.com/alphagov/city-frontend/pull/732))
...
- Removed some duplicated CSS rules from the outputted CSS
  ([PR #727](https://github.com/alphagov/city-frontend/pull/727))
...
- Fixes a bug in IE8 where the button component did not have a shadow, by
  ...
  support box-shadow
  ([PR #737](https://github.com/alphagov/city-frontend/pull/737))
...
- You can now override the helpers used to generate image and font urls, for
  ...
  You can do this by setting `$city-image-url-function` and
  `$city-font-url-function` to the name of the function(s) you wish to use.
  See `src/settings/_assets.scss` for more information and examples.
  ([PR #733](https://github.com/alphagov/city-frontend/pull/733))

- Add boilerplate template, which is a Nunjucks replacement for [GOV.UK Template](https://github.com/alphagov/city_template).
  ([PR #731](https://github.com/alphagov/city-frontend/pull/731))

🏠 Internal:

- Improve release steps, based on doing a release
  ([PR #725](https://github.com/alphagov/city-frontend/pull/725))

- Fix majority of vulnerabilities flagged by `npm audit`
  ([PR #712](https://github.com/alphagov/city-frontend/pull/712))
...
- Update component readme files content
  ([PR #704](https://github.com/alphagov/city-frontend/pull/704))

- Fix panel body element rendering with no text
  ([PR #707](https://github.com/alphagov/city-frontend/pull/707))

🆕 New features:

- Add override classes to set `display` property to `block`, `inline` and `inline-block` (PR [#694](https://github.com/alphagov/city-frontend/pull/654))

- Add option to set CSS display property for city-shape-arrow mixin
  ([PR #701](https://github.com/alphagov/city-frontend/pull/701))

- Add header component (PR [#695](https://github.com/alphagov/city-frontend/pull/695))

- Add inset text component (PR [#709](https://github.com/alphagov/city-frontend/pull/709))

- Add ability to pass explicit name value to checkbox items (PR [#719](https://github.com/alphagov/city-frontend/pull/719))

🏠 Internal:

- Run tests in pre-release
  ([PR #706](https://github.com/alphagov/city-frontend/pull/706))

- Fix CI timeouts: run Jest single thread in TravisCI
  ([PR #712](https://github.com/alphagov/city-frontend/pull/712))

- Ensure 'after' tests are not ignored
  ([PR #720](https://github.com/alphagov/city-frontend/pull/720))
...
- Restructure project to enable it to be published as a single package
  ...
  The [new project structure](<(./src/README.md)>) matches our ITCSS inspired layers and is published as a single package as `@city-frontend/frontend`.

  You will need to:

  - Update your npm dependencies to use `@city-frontend/frontend`
  ...
  ([PR #680](https://github.com/alphagov/city-frontend/pull/680))
...
- Error Messages and Hints have been moved out Label and Fieldset components.
  ...
  ```HTML
  <label class="city-c-label" for="national-insurance-number">
    National Insurance number
    <span class="city-c-label__hint">
      It’s on your National Insurance card, benefit letter, payslip or P60. For example, ‘QQ 12 34 56 C’.
    </span>
    <span class="city-c-error-message">
      Error message goes here
    </span>
  </label>
  <input class="city-c-input city-c-input--error" id="national-insurance-number" name="national-insurance-number" type="text">
  ```

  With the new markup:

  ```HTML
  <label class="city-label" for="national-insurance-number">
    National Insurance number
  </label>
  <span id="national-insurance-number-hint" class="city-hint">
    It’s on your <i>National Insurance card</i>, benefit letter, payslip or P60. For example, ‘QQ 12 34 56 C’.
  </span>
  <span id="national-insurance-number-error" class="city-error-message">
    Error message goes here
  </span>
  <input class="city-input city-input--error" id="national-insurance-number" name="test-name-3" type="text" aria-describedby="national-insurance-number-hint national-insurance-number-error">
  ```
  ...
  For more examples of the new markup and Nunjucks macros see the GOV.UK Design System [Question Pages pattern](https://city-design-system-production.cloudapps.digital/patterns/question-pages/)

  ([PR #681](https://github.com/alphagov/city-frontend/pull/681))
...
- The label component macro now accepts an `isPageHeading` argument which determines
  ...
  ```html
  <h1 class="city-label-wrapper">
    <label class="city-label">What is your National Insurance number?</label>
  </h1>
  ```

  The `.city-label-wrapper` removes the margins from the heading so that the
  presence of the `h1` has no effect on the styling of the label.

  ([PR #684](https://github.com/alphagov/city-frontend/pull/684))
...
- Added new modifier classes for labels to allow you to create a label that
  ...
  `.city-label--xl` will have the same font size and weight as a
  `.city-heading-xl`)
...
- The fieldset component has a new parameter legend.isPageHeading, which defines
  ...
  ```html
  <legend class="city-fieldset__legend">
    <h1 class="city-fieldset__heading">Have you changed your name?</h1>
  </legend>
  ```

  The `.city-fieldset__heading` class ensures that the `<h1>` inherits its
...
- Added new modifier classes for legends to allow you to create a legend that
  visually corresponds to the equivalent heading class (for example, a
  `.city-fieldset__legend--xl` will have the same font size and weight as a
  `.city-heading-xl`)

  ([PR #684](https://github.com/alphagov/city-frontend/pull/684))

- Remove -c -o -h layer prefixes
  ([PR #644](https://github.com/alphagov/city-frontend/pull/644))
...
- Simplify grid syntax and introduce grid-row and column mixins.
  ...
  ```HTML
  <div class="city-o-grid">
    <div class="city-o-grid__item city-o-grid__item--two-thirds">
      <!-- content -->
    </div>
    <div class="city-o-grid__item city-o-grid__item--one-third">
      <!-- content -->
    </div>
  </div>
  ```
  ...
  ([PR #665](https://github.com/alphagov/city-frontend/pull/665))
...
- Remove redundant font-family declaration from the button component – this will
  ...
  (PR [#650](https://github.com/alphagov/city-frontend/pull/650))

- Reinstate focus outline for radios and checkboxes on IE8
  (PR [#670](https://github.com/alphagov/city-frontend/pull/670))

- Fix section break styles when used with GOV.UK Elements
  ([PR #682](https://github.com/alphagov/city-frontend/pull/682))

- Remove `-webkit-tap-highlight-color` from links (PR [#692](https://github.com/alphagov/city-frontend/pull/692))

- Remove normalize from /dist builds
  (PR [#699](https://github.com/alphagov/city-frontend/pull/699))

🆕 New features:

- We're now using ES6 Modules and [rollup](https://rollupjs.org/guide/en) to distribute our JavaScript. (PR [#652](https://github.com/alphagov/city-frontend/pull/652))
- Checkboxes and Radios conditional reveal
  (PR [#616](https://github.com/alphagov/city-frontend/pull/616))

- Vendor-in SassMQ functionality, write tests and remove external dependency
  (PR [#657](https://github.com/alphagov/city-frontend/pull/657))

- Focus Error Summary on window load
  (PR [#671](https://github.com/alphagov/city-frontend/pull/671))

- Opt-in Global `<a>` and `<p>` styles
  (PR [#658](https://github.com/alphagov/city-frontend/pull/658))
  ...
  Hovever, we do include them in the [GOV.UK Prototype Kit](https://github.com/alphagov/city-prototype-kit-private-beta) to speed up prototyping.

  To include global styles, you can set `$city-global-styles` variable to `true`.

🏠 Internal:

- Remove unused step in travis.yml file
  (PR [#690](https://github.com/alphagov/city-frontend/pull/690))
- Update publishing docs (PR [#651](https://github.com/alphagov/city-frontend/pull/651))
- Wrap `app.css` in conditional comments in review app layout (PR [#653](https://github.com/alphagov/city-frontend/pull/653))
- Fix missing code highlight and remove duplicate layout
  (PR [#663](https://github.com/alphagov/city-frontend/pull/663))
- Exclude test related files from `dist/` and `packages/` copy task
  (PR [#662](https://github.com/alphagov/city-frontend/pull/662))
- Add test to check if Sass in packages compiles correctly after the `build:packages` task
  (PR [#669](https://github.com/alphagov/city-frontend/pull/669))
- Disable code syntax highlighting in IE8
  (PR [#675](https://github.com/alphagov/city-frontend/pull/675))
- Simplify contents of the `dist/` folder
  (PR [#673](https://github.com/alphagov/city-frontend/pull/673))
...
- Removed an (undocumented) modifier `city-c-radio__item--inline` which made
  radio buttons inline, in favour of a new block-level modifier
  `city-c-radios--inline` which will automatically make all the radio buttons
  within that block inline.
  (PR [#607](https://github.com/alphagov/city-frontend/pull/607))
- Prefix `$global-images` variable
  (PR [#615](https://github.com/alphagov/city-frontend/pull/615))
- Simplified how we build stylesheets that target old versions of IE:
  ...
  - Automatically set $mq-responsive based on the value of the $city-is-ie8
    variable.

  If you are building an IE8 stylesheet for your application you'll need to
  update it to reference the new variable $city-is-ie8 – see the README for
  details.
  (PR [#631](https://github.com/alphagov/city-frontend/pull/631))

- Rename captionSize table argument to captionClasses ([PR #643](https://github.com/alphagov/city-frontend/pull/643))
...
- Link styles, as well as links within the back-link, breadcrumbs, button,
  ...
  (PR [#609](https://github.com/alphagov/city-frontend/pull/609))
- Fix table captions to allow heading classes
  (PR [#633](https://github.com/alphagov/city-frontend/pull/633))

🆕 New features:

- Add `city-main-wrapper--l` a variant of the main page wrapper to use when a
  design does not include back links, breadcrumbs or phase banners
  (PR [#602](https://github.com/alphagov/city-frontend/pull/602))
- Added a new `.city-link--text-colour` modifier to make links use the same
  colour as the text.
  (PR [#609](https://github.com/alphagov/city-frontend/pull/609))
...
- Add limited width inputs
  (PR [#626](https://github.com/alphagov/city-frontend/pull/626))
- Add details polyfill. This is automatically initialised in the main JS. (PR [#610](https://github.com/alphagov/city-frontend/pull/610))
- Add button polyfill. This is automatically initialised in the main JS. [#572](https://github.com/alphagov/city-frontend/pull/572))
...
- Update check script for new components and tweak docs
  (PR [#589](https://github.com/alphagov/city-frontend/pull/589))
- Listen for development server on different port for tests
  (PR [#622](https://github.com/alphagov/city-frontend/pull/622))

- Fix date-input default example
  (PR [#623](https://github.com/alphagov/city-frontend/pull/623))
- Ensure we're testing error-messages in date-input
  (PR [#625](https://github.com/alphagov/city-frontend/pull/625))

- Explain npm link
  (PR [#624](https://github.com/alphagov/city-frontend/pull/624))

- Make Sass imports less broad
  (PR [#617](https://github.com/alphagov/city-frontend/pull/617))

- Update project README with user research findings (PR [#617](https://github.com/alphagov/city-frontend/pull/615))

- Update component READMEs to use `import` statement (PR [#615](https://github.com/alphagov/city-frontend/pull/615))

- Use promisify util instead of manual method (PR [#639](https://github.com/alphagov/city-frontend/pull/639))
...
- The error summary component now has a default bottom margin
  (PR [#583](https://github.com/alphagov/city-frontend/pull/583))
- Nest components correctly
  (PR [#584](https://github.com/alphagov/city-frontend/pull/584))

🔧 Fixes:

- Make section break line 1px instead of 2px
  (PR [#585](https://github.com/alphagov/city-frontend/pull/585))

🆕 New features:

- Add footer component (PR [#569](https://github.com/alphagov/city-frontend/pull/569))

🏠 Internal:

- Update pre-release step to check for new components
  (PR [#574](https://github.com/alphagov/city-frontend/pull/574))
- Ensure render function does not have undefined object
  (PR [#587](https://github.com/alphagov/city-frontend/pull/587))
- Fix components relying on global builds
  (PR [#588](https://github.com/alphagov/city-frontend/pull/588))

## 0.0.25-alpha (Breaking release)

💥 Breaking changes:

- The colour variable `$city-fuschia` has been replaced with a new variable
  with the correct spelling (`$city-fuchsia`) – thanks to
  [@charlesrt](https://github.com/charlesrt) for reporting.
  (PR [#571](https://github.com/alphagov/city-frontend/pull/571))

🆕 New features:

- The input macro now accepts a `type` which allows you to override the default
  type="text" (PR [#568](https://github.com/alphagov/city-frontend/pull/568))
...
- The transparent outline has been removed from the button, as it already has
  ...
  (PR [#552](https://github.com/alphagov/city-frontend/pull/552))
- Redundant duplication on the start button variant has been removed as it is
  inherited from the main .city-c-button class
  (PR [#552](https://github.com/alphagov/city-frontend/pull/552))
...
- The date input component now sets `type="number"` and `pattern="[0-9]*"` to
  ...
  (PR [#568](https://github.com/alphagov/city-frontend/pull/568))

🏠 Internal:

- The logic to determine button text colour automatically has been removed and
  replaced with a new variable $city-button-text-colour
  (PR [#552](https://github.com/alphagov/city-frontend/pull/552))
...
- Fixes for the release process
  (PR [#567](https://github.com/alphagov/city-frontend/pull/567))
- Add HTML5-shiv to review app
  (PR [#575](https://github.com/alphagov/city-frontend/pull/575))

## 0.0.24-alpha (Breaking release)

💥 Breaking changes:

- Namespace existing mixins and functions ( PR [#557](https://github.com/alphagov/city-frontend/pull/557))
- The class `.city-section-break__visible` has been renamed to
  `.city-section-break--visible` as it is a modifier, not an element.
  (PR [#547](https://github.com/alphagov/city-frontend/pull/547))
- Simplify `src/` folder structure, remove tree and string manipulations from
  gulp tasks (PR [#545](https://github.com/alphagov/city-frontend/pull/545))

🆕 New features:

- A new variable `$city-input-border-colour` has been introduced to define the
  border colour for inputs. The Input, Select and Textarea components have been
  updated to use it.
  (PR [#551](https://github.com/alphagov/city-frontend/pull/551))

🔧 Fixes:

- Removes media query display on body from compiled CSS
  (PR [#560](https://github.com/alphagov/city-frontend/pull/560))

- Fieldset legends now correctly use 'full black' text colour when printed
  (PR [#544](https://github.com/alphagov/city-frontend/pull/544))
...
- The 'prose scope' has been updated to extend only placeholder classes. The
  ...
  (PR [#550](https://github.com/alphagov/city-frontend/pull/550))
...
- Release process has been streamlined with fewer steps
  (PR [#553](https://github.com/alphagov/city-frontend/pull/553))
- Update `city-section-break__visible` new class name `city-section-break--visible` in the review app's Typography page.
  (PR [#566](https://github.com/alphagov/city-frontend/pull/566))
...
- Remove Cookie-banner and Previous-next components
  (PR [#488](https://github.com/alphagov/city-frontend/pull/488), PR [#523](https://github.com/alphagov/city-frontend/pull/523))
- Remove unused colours from the 'applied' colour palette:
  (PR [#525](https://github.com/alphagov/city-frontend/pull/525))
  - `$city-panel-colour`
  - `$city-canvas-colour`
  ...
  - `$city-footer-text`
- Rename `$city-button-colour-darken-15` to `$city-button-shadow-colour` to
  avoid tying it to a particular colour.
  (PR [#525](https://github.com/alphagov/city-frontend/pull/525))

🆕 New features:

- Button hover colour now has a semantic Sass name: $city-button-hover-colour
  (PR [#406](https://github.com/alphagov/city-frontend/pull/406))
- A new link variant has been added which removes the visited state, for cases
  ...
  (PR [#446](https://github.com/alphagov/city-frontend/pull/446))
- The responsive spacing and typography mixins will now throw an error if you
  ...
  (PR [#447](https://github.com/alphagov/city-frontend/pull/447))
- Add thematic break to typography and prose scope. This class is added to an
  ...
  border visible or invisible. (PR [#483](https://github.com/alphagov/city-frontend/pull/483))

🔧 Fixes:

- Remove double margin from Date input component
  (PR [#451](https://github.com/alphagov/city-frontend/pull/451))
- Add top margin for nested lists (PR [#464](https://github.com/alphagov/city-frontend/pull/464))
- Remove regular font weight from link styles (PR [#469](https://github.com/alphagov/city-frontend/pull/469))
- Remove redundant 'city-c-border' div from the details component
  (PR [#481](https://github.com/alphagov/city-frontend/pull/481))
- Add `city-c-select--error` modifier class to the select component instead of relying on `city-c-input--error` (PR [#506](https://github.com/alphagov/city-frontend/pull/506))
- Allow error message and hint text to be passed to a select component without requiring a label parameter (PR [#506](https://github.com/alphagov/city-frontend/pull/506))
- Define size of inputs etc in `px` rather than `em`. (PR [#491](https://github.com/alphagov/city-frontend/pull/491))
- Remove scope=row attribute from non-th elements (PR [527](https://github.com/alphagov/city-frontend/pull/527))
- Form components and fieldset now include `city-o-form-group` that sets left
  border for errors and a bottom margin. Add example of form errors to preview app
  (PR [#591](https://github.com/alphagov/city-frontend/pull/591))
...
- Replace Mocha/Chai with Jest, re-enable task tests, add back-link component.
  ...
  (PR [#455](https://github.com/alphagov/city-frontend/pull/455))
- Add example of nested lists to typography and prose scope in review app (PR [#464](https://github.com/alphagov/city-frontend/pull/464))
- Add tests for tag component (PR [#457](https://github.com/alphagov/city-frontend/pull/457))
- Add tests for button component (PR [#461](https://github.com/alphagov/city-frontend/pull/461))
- Add tests for breadcrumbs component (PR [#461](https://github.com/alphagov/city-frontend/pull/461))
- Add tests for details component (PR [#480](https://github.com/alphagov/city-frontend/pull/480))
- Add tests for warning text component (PR [#479](https://github.com/alphagov/city-frontend/pull/479))
- Add tests for table component (PR [#472](https://github.com/alphagov/city-frontend/pull/472))
- Add tests for error-summary component (PR [#489](https://github.com/alphagov/city-frontend/pull/489))
- Add tests for radios component (PR [#476](https://github.com/alphagov/city-frontend/pull/476))
- Add tests for input component (PR [#478](https://github.com/alphagov/city-frontend/pull/478))
- Add tests for date-input component (PR [#495](https://github.com/alphagov/city-frontend/pull/495))
- Add tests for textarea component (PR [#497](https://github.com/alphagov/city-frontend/pull/497))
- Add tests for panel component (PR [#500](https://github.com/alphagov/city-frontend/pull/500))
- Add tests for skip-link component (PR [#498](https://github.com/alphagov/city-frontend/pull/498))
- Add tests for file-upload component (PR [#504](https://github.com/alphagov/city-frontend/pull/504))
- Add tests for error-message component (PR [#507](https://github.com/alphagov/city-frontend/pull/507))
- Add tests for phase-banner component (PR [#505](https://github.com/alphagov/city-frontend/pull/505))
- Add tests for label component component (PR [#508](https://github.com/alphagov/city-frontend/pull/508))
- Add tests for fieldset component (PR [#509](https://github.com/alphagov/city-frontend/pull/509))
- Add tests for select component (PR [#506](https://github.com/alphagov/city-frontend/pull/506))
- Add tests for checkboxes component (PR [#513](https://github.com/alphagov/city-frontend/pull/513))
- Add tests to make sure the examples pages render without errors [#523](https://github.com/alphagov/city-frontend/pull/523)
- Add correct links to the guidance in the Design System in component READMEs
  (PR [#528](https://github.com/alphagov/city-frontend/pull/528))
...
- The link styles in the core layer no longer style `a` elements directly, but
  instead provide a `city-link` class which you will need to apply to links
  individually. (PR [#427](https://github.com/alphagov/city-frontend/pull/427))
- The link component has been removed from Frontend as the link styles have
  been moved to the core
  (PR [#431](https://github.com/alphagov/city-frontend/pull/431))
- Rename `legal-text` argument accepted by `legal-text` component to `text` (PR [#431](https://github.com/alphagov/city-frontend/pull/432))
- Rename `legal-text` component to `warning-text` (PR [#431](https://github.com/alphagov/city-frontend/pull/432))

🆕 New features:

- The prose scope has been extended to style links, which means links within the
  scope do not need the `city-link` class applied.
  (PR [#427](https://github.com/alphagov/city-frontend/pull/427))
- The muted link variant from the link component is now available as a core
  class (`city-link--muted`).
  (PR [#427](https://github.com/alphagov/city-frontend/pull/427))

🔧 Fixes:

- The error summary component allows users to pass HTML for an entry in the list
  again. (PR [#428](https://github.com/alphagov/city-frontend/pull/428))
- Error list entries in the error summary component no longer get wrapped in
  links when no `href` is provided.
  (PR [#428](https://github.com/alphagov/city-frontend/pull/428))
...
- Add prose scope example (PR [#429](https://github.com/alphagov/city-frontend/pull/429))
- Links within the review app and the examples have been updated to use the
  `city-link` class.
  (PR [#427](https://github.com/alphagov/city-frontend/pull/427))
- Improve documentation around publishing (PR [#430](https://github.com/alphagov/city-frontend/pull/430))
- Improve documentation around contributing (PR [#433](https://github.com/alphagov/city-frontend/pull/433))
- Remove readme content from review app (PR [#482](https://github.com/alphagov/city-frontend/pull/482))
...
- Rename ‘city-body-lede’ to ‘city-body-lead’. (PR [#405](https://github.com/alphagov/city-frontend/pull/405))
- Pluralise radio component (PR [#388](https://github.com/alphagov/city-frontend/pull/388))
- Pluralise checkbox component (PR [#384](https://github.com/alphagov/city-frontend/pull/384))

🆕 New features:

- Add `width` classes, which were based on `form-control` classes that were
  specific to form control in Elements. (PR [#413](https://github.com/alphagov/city-frontend/pull/413))

🔧 Fixes:

- Make buttons 40px high including box shadow (PR [#416](https://github.com/alphagov/city-frontend/pull/416))
- Fix focus outline style in Chrome and Safari (PR [#414](https://github.com/alphagov/city-frontend/pull/414))
- Remove contributors list from template, fix template markup and update README files (PR [#403](https://github.com/alphagov/city-frontend/pull/403))
- Generate breadcrumb chevrons using pseudo-elements (PR [#407](https://github.com/alphagov/city-frontend/pull/407))
- Fix undefined classes in date input macro (PR [#410](https://github.com/alphagov/city-frontend/pull/410))
- Add documentation for typography helpers / core, simplify syntax (PR [#400](https://github.com/alphagov/city-frontend/pull/400))
- Add adjacent styles for headings after lists (PR [#408](https://github.com/alphagov/city-frontend/pull/408))
- Allow line height to be overridden in typography helpers (PR [#404](https://github.com/alphagov/city-frontend/pull/404))
