...
changes="$(git status --porcelain -- ':!CHANGELOG.md' ':!packages/city-frontend/package.json' ':!package-lock.json')"
if [[ -n $changes ]]; then
...
echo "- build GOV.UK Frontend into the 'packages/city-frontend/dist/' directory"
